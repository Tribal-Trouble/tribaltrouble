...
public final class GeneratorOcean extends TextureGenerator {
    ...
    public Texture @NonNull [] generate() {
        ...
        Channel perlin2 = new Perlin(TEXTURE_SIZE, TEXTURE_SIZE, 4, 4, 0.5f, 1, seed, Interpolation.CUBIC,
                Summation.NORMAL).toChannel();
        Channel perlin4 = new Perlin(TEXTURE_SIZE, TEXTURE_SIZE, 4, 4, 0.5f, 3, seed, Interpolation.CUBIC,
                Summation.NORMAL).toChannel();
        Channel perlin32 = new Perlin(TEXTURE_SIZE, TEXTURE_SIZE, 32, 32, 0.5f, 2, seed, Interpolation.CUBIC,
                Summation.NORMAL).toChannel();
        Channel perlin64 = new Perlin(TEXTURE_SIZE, TEXTURE_SIZE, 64, 64, 0.5f, 2, seed, Interpolation.CUBIC,
                Summation.NORMAL).toChannel();
        Channel noise32 = perlin32.copy().abs().dynamicRange().gamma8().gamma2().invert().perturb(perlin4.copy().rotate(
                90), 0.05f).perturb(perlin2, 0.05f);
