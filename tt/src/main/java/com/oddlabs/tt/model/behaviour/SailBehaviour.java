...
public final class SailBehaviour implements Behaviour {
    ...
    class TrajectoryPoint {
        ...
        public TrajectoryPoint(int x, int y, float dx, float dy) {
            gridX = x;
            gridY = y;
            positionX = UnitGrid.coordinateFromGrid(gridX);
            positionY = UnitGrid.coordinateFromGrid(gridY);
    ...
    public SailBehaviour(Ship ship, Target t, float range) {
        ...
        if (checkCollisionOnLine(p0, p0.moved(20), 6)) {
            for (int i = 0; i < 12; i++) {
                float angle = i * 15.0f;
                TrajectoryPoint gap0 = getNearestGap(p0, p0.rotated(angle).moved(40), 20, 50, 6);
                TrajectoryPoint gap1 = getNearestGap(p0, p0.rotated(-angle).moved(40), 20, 50, 6);
                if (gap0 != null && gap1 != null) {
                    float dist0 = gap0.gridDistanceTo(p0);
                    float dist1 = gap1.gridDistanceTo(p0);
                    if (dist0 > dist1) {
                        firstGap = gap0;
                    } else {
                        firstGap = gap1;
            ...
            firstGap = p0.moved(10);
        }

        if (checkCollisionOnLine(p1.moved(-4), p1.moved(4), 8)) {
            for (int i = 0; i < 12; i++) {
                float angle = i * 15.0f;
                TrajectoryPoint gap0 = getNearestGap(p1, p1.rotated(angle).moved(20), 10, 15, 6);
                TrajectoryPoint gap1 = getNearestGap(p1, p1.rotated(-angle).moved(20), 10, 15, 6);
                if (gap0 != null && gap1 != null) {
                    float dist0 = gap0.gridDistanceTo(p1);
                    float dist1 = gap1.gridDistanceTo(p1);
                    if (dist0 > dist1) {
                        lastGap = gap0;
                    } else {
                        lastGap = gap1;
    ...
    public @NonNull State animate(float t) {
        ...
        ship.setGridPosition(new_x, new_y);
        ship.setDirection(new_pose.z, new_pose.w);
        ship.occupy();

        if (new_x == target.getGridX() && new_y == target.getGridY()) {
            ship.endTrip();
            return State.DONE;
        }
