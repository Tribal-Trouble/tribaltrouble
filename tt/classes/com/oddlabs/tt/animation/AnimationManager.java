...
public final strictfp class AnimationManager {
    ...
    public static final int ANIMATION_MILLISECONDS_PER_PRECISION_TICK =
            ANIMATION_MILLISECONDS_PER_TICK / 5;
    public static final float ANIMATION_SECONDS_PER_TICK = ANIMATION_MILLISECONDS_PER_TICK / 1000f;
    public static final float ANIMATION_SECONDS_PER_PRECISION_TICK =
            ANIMATION_MILLISECONDS_PER_PRECISION_TICK / 1000f;
    private static final int ANIMATION_MILLISECONDS_PER_CHECKSUM = 2000;
    public static final int MAX_STEP_MILLIS = 30000;

    public static final StatCounter pathfinds_per_tick = new StatCounter(100);

    private static final StatCounter frame_time = new StatCounter(10);
    private static final MonotoneTimeManager time_source;
    ...
    */
    ...
    public static final void warpTime(long warp_delta) {
    ...
    public static final long getSystemTime() {
        ...
        } else return time_source.getMillis() + time_warp;
    }

    public static void toggleTimeStop() {
        if (time_frozen) unfreezeTime();
        else freezeTime();
        time_stopped = time_frozen;
        System.out.println("time_stopped = " + time_stopped);
    }

    private static final void unfreezeTime() {
        if (!time_frozen) return;
        time_frozen = false;
        time_warp -= time_source.getMillis() - frozen_start_time;
    }

    public static final void freezeTime() {
        if (time_frozen) return;
    ...
    public static final void runGameLoop(NetworkSelector network, GUI gui, boolean grab_frames) {
        KeyboardInput.checkMagicKeys();
        if (time_frozen && !time_stopped) unfreezeTime();
        if (grab_frames) {
        ...
        while (execution_time_precision >= ANIMATION_MILLISECONDS_PER_PRECISION_TICK
                && !Renderer.isFinished()) {
            /*
            ...
            while (execution_time >= ANIMATION_MILLISECONDS_PER_TICK && !Renderer.isFinished()) {
                ...
                if (checksum_millisecond_counter >= ANIMATION_MILLISECONDS_PER_CHECKSUM) {
                    ...
                    if (checksum != logged_checksum && checksum_complain) {
                        System.out.println(
                                "********** ERROR: Checksum mismatch at tick "
    ...
    public final void runAnimations(float t) {
        ...
        for (int i = 0; i < animations.size(); i++) {
            ...
            if (deleted_animations.contains(current)) continue;
            current.animate(t);
        }
    }
