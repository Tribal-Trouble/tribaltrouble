...
public abstract class Selectable extends Model implements Target, Animated, ModelToolTip {
    ...
    public final void pushControllers(Controller[] controllers) {
        assert !isDead();
        for (int i = 0; i < controllers.length; i++) controller_stack.add(controllers[i]);
        decide();
    }
    ...
    public final Controller getPrimaryController() {
        ...
        else return (Controller) controller_stack.get(0);
    }
    ...
    public final void setGridPosition(int grid_x, int grid_y) {
        assert !isDead();
        assert owner.getWorld().getHeightMap().isGridInside(grid_x, grid_y)
                : grid_x + " " + grid_y + " " + this.grid_x + " " + this.grid_y;
        this.grid_x = grid_x;
        this.grid_y = grid_y;
    }
