...
public final strictfp class Building extends Selectable implements Occupant, Movable {
    ...
    protected final void doAnimate(float t) {
        ...
        if (remove_delay > 0) {
            ...
            if (remove_delay <= 0) {
                ...
                if (weapons_producer != null) weapons_producer.stopSound();
                float energy = 3f;
    ...
    public final boolean canExitTower() {
        return !isDead()
                ...
                && getUnitContainer().getNumSupplies() > 0
                && getOwner().canExitTowers()
                && !(((MountUnitContainer) getUnitContainer()).getUnit().getCurrentController()
                        instanceof StunController);
    }
    ...
    public final void buildWeapons(Class type, int num_weapons, boolean infinite) {
        ...
        ((BuildProductionContainer) getBuildSupplyContainer(type))
                .orderSupply(num_weapons, infinite);
    }
    ...
    private final Unit createUnit(Target rally_point, UnitTemplate template) {
        if (getAbilities().hasAbilities(Abilities.SAIL)) {
            return ship_hr.exitUnit(template);
        } else {
            return new Unit(
                    getOwner(),
                    getPositionX(),
    ...
    private final void createTransporters(int amount, Class supply) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            // TODO: When transporting from a ship, deploy one of the peons
            // TODO: Also check if there are any peons
        ...
        if (build_points < getBuildingTemplate().getMaxHitPoints()) {
            ...
            if (build_points == getBuildingTemplate().getMaxHitPoints()) {
                ...
                if (getAbilities().hasAbilities(Abilities.SUPPLY_CONTAINER)) {
                    ...
                    BuildProductionContainer rock_axe_weapon =
                            new BuildProductionContainer(
                                    BuildSpinner.INFINITE_LIMIT,
                                    rock_weapon_container,
                                    this,
                                    COST_ROCK_WEAPON,
                                    40f);
                    BuildProductionContainer iron_axe_weapon =
                            new BuildProductionContainer(
                                    BuildSpinner.INFINITE_LIMIT,
                                    ...
                                    COST_IRON_WEAPON,
                                    80f);
                    BuildProductionContainer rubber_axe_weapon =
                            new BuildProductionContainer(
                                    BuildSpinner.INFINITE_LIMIT,
    ...
    protected final void setTarget(Target target, int action, boolean aggressive) {
        if (getAbilities().hasAbilities(Abilities.SAIL)) {
            ...
            if (target != this) {
                ...
                if (action == Target.ACTION_ATTACK) kill_friendly = true;
        ...
        if (worker_container != null) {
            int result =
                    getOwner()
                            ...
                            .increaseSupply(-worker_container.getNumSupplies());
    ...
    public final void printDebugInfo() {
        ...
        if (getAbilities().hasAbilities(Abilities.REPRODUCE)) {
            ...
            System.out.println(
                    "Rubber = " + getSupplyContainer(RubberSupply.class).getNumSupplies());
            System.out.println(
