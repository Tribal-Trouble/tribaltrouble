...
public strictfp class Unit extends Selectable implements Occupant, Movable {
    ...
    public Unit(
        ...
        pushController(
                new IdleController(
                        this, new AttackScanFilter(getOwner(), AttackScanFilter.UNIT_RANGE), true));
    ...
    public final int getAttackPriority() {
        assert !isDead();
        if (getAbilities().hasAbilities(Abilities.BUILD)) return AttackScanFilter.PRIORITY_PEON;
        else return AttackScanFilter.PRIORITY_WARRIOR;
    }
    ...
    public final float getMetersPerSecond() {
        ...
        else return getUnitTemplate().getMetersPerSecond();
    }
    ...
    public final void markBlocking() {
        assert !isDead();
        path_penalty =
                StrictMath.min(path_penalty + PENALTY_INCREMENT, STATIC - 1); // never gets STATIC
    }

    public final int getPenalty() {
        assert !isDead();
        if (isBlocking()) return Occupant.STATIC;
        else return path_penalty;
    }
    ...
    */
    ...
    public final void stun(float time) {
        float x =
                getPositionX()
                        + getUnitTemplate().getStunX() * getDirectionX()
                        + getUnitTemplate().getStunY() * (-getDirectionY());
        float y =
                getPositionY()
                        + getUnitTemplate().getStunX() * getDirectionY()
                        + getUnitTemplate().getStunY() * getDirectionX();
        float z =
                getOwner().getWorld().getHeightMap().getNearestHeight(x, y)
                        + getUnitTemplate().getStunZ()
                        + mount_offset;
    ...
    private final BalancedParametricEmitter createStunStar(
            float x, float y, float z, float time, float velocity) {
        int num_particles = 5;
        return new BalancedParametricEmitter(
                getOwner().getWorld(),
                new StunFunction(.4f, .15f),
                new Vector3f(x, y, z),
                velocity,
                5f,
    ...
    private final boolean canRepair(Target target, boolean action_repair) {
        ...
        if (!action_repair
                ...
                && building.isComplete()) return false;
        // return getOwner() == building.getOwner() && building.isPlaced() && building.isDamaged();
        return !getOwner().isEnemy(building.getOwner())
                && building.isPlaced()
                && building.isDamaged();
    }
    ...
    public final boolean canDoMagic(int magic_index) {
        return !isDead()
                && magic_index >= 0
                && magic_index < RacesResources.NUM_MAGIC
                && getOwner().canDoMagic(magic_index)
                && magic_energy[magic_index] == MAX_MAGIC_ENERGY[magic_index];
    }

    public final void doMagic(int magic_index, boolean clear_stack) {
        if (canDoMagic(magic_index)) {
            if (clear_stack) clearControllerStack();
            pushController(
                    new MagicController(this, getOwner().getRace().getMagicFactory(magic_index)));
            for (int i = 0; i < magic_energy.length; i++) {
                magic_energy[i] = 0f;
            }
