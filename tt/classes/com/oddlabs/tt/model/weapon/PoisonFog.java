...
public final strictfp class PoisonFog implements Magic {
    ...
    public PoisonFog(
        ...
        bubbling_sound =
                owner.getWorld()
                        ...
                        .newAudio(
                                new AudioParameters(
                                        owner.getWorld().getRacesResources().getBubblingSound(),
                                        start_x,
                                        start_y,
                                        owner.getWorld()
                                                .getHeightMap()
                                                .getNearestHeight(start_x, start_y),
    ...
    public final void animate(float t) {
        ...
        if (bursts * SECONDS_BETWEEN_BURSTS < time) {
            ...
            new RandomVelocityEmitter(
                    ...
                    new Vector3f(x, y, z),
                    OFFSET_Z,
                    owner.getWorld().getRandom().nextFloat() * (float) StrictMath.PI * 2,
                    BURST_RADIUS,
                    0f,
                    0f,
                    0f,
            ...
            if (bursts % next_sound == 0) {
                ...
                owner.getWorld()
                        ...
                        .newAudio(
                                new AudioParameters(
                                        owner.getWorld().getRacesResources().getGasSound(),
                                        x,
                                        y,
                                        z,
    ...
    private final void hitUnits(float radius) {
        FindOccupantFilter filter =
                new FindOccupantFilter(start_x, start_y, radius, src, Unit.class);
        UnitGrid unit_grid = owner.getWorld().getUnitGrid();
        unit_grid.scan(
                filter,
                unit_grid.toGridCoordinate(start_x),
                unit_grid.toGridCoordinate(start_y),
                UnitGrid.LAND);
        ...
        for (int i = 0; i < target_list.size(); i++) {
            ...
            if (!s.isDead()
                    && ((owner.isEnemy(s.getOwner())
                                    && owner.getWorld().getRandom().nextFloat()
                                            < hit_chance * (1 - s.getDefenseChance()))
                            || (!owner.isEnemy(s.getOwner())
                                    && owner.getWorld().getRandom().nextFloat()
                                            < (hit_chance / 4f) * (1 - s.getDefenseChance())
    ...
    public final void interrupt() {
        if (bubbling_sound != null) bubbling_sound.stop(.2f, Settings.getSettings().sound_gain);
    }
}
