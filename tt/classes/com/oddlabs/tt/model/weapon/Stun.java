...
public final strictfp class Stun implements Magic {
    ...
    public Stun(
            ...
            float offset_z,
            float hit_radius,
            float stun_time_closest,
            float stun_time_farthest,
            Unit src) {
        ...
        emitter =
                new RandomVelocityEmitter(
                        owner.getWorld(),
                        new Vector3f(start_x, start_y, z),
                        0f,
                        0f,
                        .001f,
                        .001f,
        ...
        //		FindOccupantFilter filter = new FindOccupantFilter(src.getPositionX(),
        // src.getPositionY(), hit_radius, src, Unit.class);
                ...
                owner.getWorld()
                        ...
                        .newAudio(
                                new AudioParameters(
                                        owner.getWorld()
                                                .getRacesResources()
                                                .getStunSound(owner.getWorld().getRandom()),
                                        start_x,
                                        start_y,
                                        z,
        ...
        for (int i = 0; i < target_list.size(); i++) {
            ...
            if (target_list.get(i) instanceof Unit) {
                ...
                if (!building.isDead() && building.getAbilities().hasAbilities(Abilities.ATTACK)) {
                    MountUnitContainer muc =
                            (MountUnitContainer) ((Building) target_list.get(i)).getUnitContainer();
            ...
            if (owner.isEnemy(unit.getOwner()) && squared_dist < hit_radius * hit_radius) {
                ...
                float time =
                        calculateValueFromCurrentRadius(
                                dist, stun_time_closest, stun_time_farthest);
                unit.stun(time);
            }
        }
        interrupt();
