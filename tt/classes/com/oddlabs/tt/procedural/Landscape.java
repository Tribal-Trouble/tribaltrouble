...
public final strictfp class Landscape {
    ...
    public static final int VIKING = 1;
    ...
    private Channel overall_access;
    ...
    public Landscape(
        ...
        Voronoi voronoi;
        Channel voronoi4 =
                new Voronoi(structure_size, 4, 4, 1, 1f, STRUCTURE_SEED).getDistance(c1, c2, c3);
    ...
    // * STRUCTURES *
    // **************
    private final void generateStructuresNative(
            Channel voronoi4,
            Channel voronoi8,
            Channel voronoi8_hit,
            Channel voronoi16,
            Channel voronoi16_hit,
            Channel voronoi32,
            Channel voronoi32_hit,
            Channel noise8,
            Channel noise256) {
        ...
        Layer structure_dirt =
                genDirt(structure_size, noise8.copy(), noise256.copy(), voronoi32.copy());
        structures[1] = new GLIntImage(structure_dirt);

        Layer structure_rubble =
                genRubble(
                        structure_size,
                        noise8.copy(),
                        voronoi4.copy(),
        ...
        Layer structure_rock =
                genRock(
                        ...
                        voronoi8.copy(),
                        voronoi16.copy(),
                        structure_rubble.copy(),
                        structure_grass.copy());
        ...
        Layer structure_detail =
                genDetail(detail_size, detail_alpha_value, STRUCTURE_SEED, noise8.copy());
        detail = new GLIntImage(structure_detail);
    }

    private final void generateStructuresViking(
            Channel voronoi4,
            Channel voronoi8,
            Channel voronoi8_hit,
            Channel voronoi16,
        ...
        Layer structure_soil =
                genSoil(structure_size, noise8.copy(), noise256.copy(), voronoi32.copy());
        structures[1] = new GLIntImage(structure_soil);

        Layer structure_grass =
                genGrass(structure_size, noise8.copy(), noise256.copy()).multiply(.75f, .9f, 1.1f);
        structures[3] = new GLIntImage(structure_grass);

        Layer structure_cliff =
                genCliff(
                        structure_size,
                        noise8.copy(),
                        noise256.copy(),
    ...
    private final Layer genSand(int size, Channel noise8, Channel noise256) {
        ...
        sand.bump(
                sand_bump1.channelAdd(sand_bump2), size / 1024f, 0f, 0.1f, 1f, 1f, 1f, 0f, 0f, 0f);
    ...
    private final Layer genGravel(int size, Channel noise8, Channel noise256) {
        ...
        Layer gravel =
                new Layer(
                        empty.copy().fill(0.7f), empty.copy().fill(0.55f), empty.copy().fill(0.4f));
        gravel.bump(
                gravel_bump1.channelAdd(gravel_bump2),
                size / 1024f,
                0f,
                0.1f,
                1f,
    ...
    private final Layer genSoil(int size, Channel noise8, Channel noise256, Channel voronoi32) {
        ...
        Layer soil =
                new Layer(
                        empty.copy().fill(.65f), empty.copy().fill(.5f), empty.copy().fill(0.35f));
    ...
    private final Layer genGrass(int size, Channel noise8, Channel noise256) {
        ...
        Layer grass =
                new Layer(empty.copy().fill(0.2f), empty.copy().fill(0.45f), empty.copy().fill(0f));
    ...
    private final Layer genRubble(
        ...
        Channel rubble_bump =
                rubble_bump1.channelAdd(rubble_bump2).channelAdd(rubble_bump3).dynamicRange();
    ...
    private final Layer genRock(
            ...
            Layer grass) {
        Channel rock_bump1 =
                voronoi4.dynamicRange().contrast(1.1f).brightness(2f).gamma2().multiply(0.35f);
        Channel rock_bump2 =
                voronoi8.dynamicRange().contrast(1.1f).brightness(2f).gamma2().multiply(0.3f);
        Channel rock_bump3 =
                voronoi16.dynamicRange().contrast(1.1f).brightness(2f).gamma2().multiply(0.2f);
        Channel rock_bump =
                rock_bump1
                        .channelAdd(rock_bump2)
                        .channelAdd(rock_bump3)
                        .channelAdd(noise256.multiply(0.15f));
    ...
    private final Layer genCliff(
            ...
            Layer grass) {
        Channel cliff_bump1 =
                voronoi4.dynamicRange().contrast(1.1f).brightness(2f).gamma2().multiply(0.3f);
        Channel cliff_bump2 =
                voronoi8.dynamicRange().contrast(1.1f).brightness(2f).gamma2().multiply(0.3f);
        Channel cliff_bump3 =
                voronoi16.dynamicRange().contrast(1.1f).brightness(2f).gamma2().multiply(0.25f);
        Channel cliff_bump =
                cliff_bump1
                        .channelAdd(cliff_bump2)
                        .channelAdd(cliff_bump3)
                        .channelAdd(noise256.multiply(0.15f));
    ...
    private final Layer genSnow(int size, Channel noise8, Channel noise256) {
        ...
        snow.bump(
                snow_bump1.channelAdd(snow_bump2), size / 1024f, 0f, 0.1f, 1f, 1f, 1f, 0f, 0f, 0f);
    ...
    private final void generateTerrainNative() {
        ...
        height =
                new Mountain(
                                ...
                                Utils.powerOf2Log2(unit_grids_per_world) - 6,
                                0.5f,
                                seed)
                        .toChannel()
                        .multiply(0.67f);
        ...
        if (archipelago) {
            Channel sub_islands =
                    new Voronoi(unit_grids_per_world, features, features, 1, 1f, seed)
                            .getDistance(-1.0f, 0.0f, 0.0f)
                            .gamma(0.11f);
        ...
        access =
                generateThresholdMap(slope, access_threshold, Globals.SEA_LEVEL)
                        .largestConnected(1f);
        overall_access =
        ...
        if (DEBUG) overall_access.toLayer().saveAsPNG("overall_access");
        build =
                generateBuildMap(
                        generateThresholdMap(slope, build_threshold, Globals.SEA_LEVEL)
                                .channelMultiply(overall_access));
    }
        ...
        for (int y = 0; y < channel.height; y++) {
            for (int x = 0; x < channel.width; x++) {
                ...
                if (value < sealevel) {
                    ...
                    value =
                            Tools.interpolateSmooth(
                                    sealevel,
                                    2f * threshold - sealevel,
    ...
    private final Channel generateThresholdMap(Channel slopemap, float threshold, float min) {
        Channel channel =
                slopemap.copy()
                        .threshold(0f, threshold)
                        .channelSubtract(height.copy().threshold(0f, min));
        // fix edges
        for (int y = 0; y < unit_grids_per_world; y += (unit_grids_per_world - 1)) {
            for (int x = 0; x < unit_grids_per_world; x++) {
    ...
    private final byte[][] generateBuildMap(Channel thresholdmap) {
        ...
        byte max =
                (byte)
                        StrictMath.max(
                                RacesResources.QUARTERS_SIZE,
                                StrictMath.max(
                                        RacesResources.ARMORY_SIZE, RacesResources.TOWER_SIZE));
    ...
    private final Channel generateDirtAlpha() {
        Channel dirt_alpha =
                height.copy()
                        .dynamicRange(1.1f * Globals.SEA_LEVEL, 2f * Globals.SEA_LEVEL, 0f, 1f);
    ...
    private final Channel generateGrassAlpha(int size, int seed) {
        Channel grass_alpha =
                new Midpoint(size, 4, 0.45f, seed)
                        .toChannel()
                        .dynamicRange(1f - vegetation_amount, 1f, 0f, 1f)
                        .gamma2();
        grass_alpha.channelBrightest(
                slope.copy()
                        .dynamicRange(0f, access_threshold, 0f, 1f)
                        .invert()
                        .dynamicRange(1f - vegetation_amount, 1f, 0f, 1f)
                        .gamma2());
    ...
    private final Channel generateSeabottomAlpha() {
        Channel seabottom_alpha =
                height.copy().invert().dynamicRange(1f - Globals.SEA_LEVEL, 1f, 0f, 1f);
        switch (terrain_type) {
            case NATIVE:
    ...
    private final void generateSupplies(Channel grass_alpha) {
        ...
        Channel centerprob =
                new Hill(unit_grids_per_world, Hill.CIRCLE)
                        .toChannel()
                        .addClip(-.5f)
                        .dynamicRange();
        ...
        Channel noise =
                new Midpoint(
                                unit_grids_per_world,
                                Utils.powerOf2Log2(unit_grids_per_world) - 3,
                                0.33f,
                                seed)
                        .toChannel();
        ...
        switch (terrain_type) {
            case NATIVE:
                ...
                palmtree_channel.channelSubtract(
                        height.copy().invert().dynamicRange(0.89f, 0.9f, 0f, 1f));
                palmtree_channel
                        .channelSubtract(noise.rotate(90))
                        .channelAdd(noise.rotate(90).copy().threshold(0.9f, 1f))
                        .channelSubtract(slope.copy().threshold(access_threshold, 1f));
                break;
            case VIKING:
                tree_channel.gamma8();
                ...
                palmtree_channel =
                        grass_alpha
                                .copy()
                                .channelMultiply(height.copy().dynamicRange(0.5f, 0.6f, 1f, 0f))
                                .invert();
                palmtree_channel.channelSubtract(
                        height.copy().invert().dynamicRange(0.8f, 0.875f, 0f, 1f));
        ...
        Channel supplies = overall_access.copy();
        ...
        trees =
                placeSupplies(
                        ...
                        supplies,
                        64,
                        (int) (vegetation_amount * max_trees * (accessible / area)),
                        0.33f);
        overall_access.channelSubtract(trees);
        ...
        palmtrees =
                placeSupplies(
                        ...
                        supplies,
                        64,
                        (int) (vegetation_amount * max_palmtrees * (accessible / area)),
                        0.25f);
        overall_access.channelSubtract(palmtrees);
        ...
        overall_access.channelSubtract(rock);
        ...
        overall_access.channelSubtract(iron);
    ...
    private final Channel placeSupplies(
            Channel probability,
            Channel supplies,
            int intervals,
            int max_count,
            float shadow_alpha_val) {
        max_count = StrictMath.min(probability.width * probability.height, max_count);
        int scaleshift =
                Utils.powerOf2Log2(
                        unit_grids_per_world * meters_per_height_unit / meters_per_world);
        int i = 0;
        ...
        Channel supplyshadow_alpha =
                new Channel(supplyshadow_size << 1, supplyshadow_size << 1)
                        .place(
                                new Channel(supplyshadow_size, supplyshadow_size).fill(1f),
                                supplyshadow_size >> 1,
                                supplyshadow_size >> 1)
                        .smoothFast();
    ...
    // place plants on map
    private final Channel placePlants(
            Channel probability, Channel place, int intervals, int max_count, int plant_type) {
        max_count = StrictMath.min(probability.width * probability.height, max_count);
        int i = 0;
        float interval_size = 1f / intervals;
        float upper_bound = 1f;
        ...
        while (i < max_count && lower_bound > interval_size) {
            for (int y = 2; y < probability.height - 2; y++) {
                for (int x = 2; x < probability.width - 2; x++) {
                    ...
                    if (val <= upper_bound
                            && val > lower_bound
                            && place.getPixel(x, y) < 2
                            && random.nextFloat() < .25) {
                        // place plant
                        plants[plant_type][2 * i] =
                                meters_per_height_unit * (x + random.nextFloat());
                        plants[plant_type][2 * i + 1] =
                                meters_per_height_unit * (y + random.nextFloat());
    ...
    private final void generateUnitLocations(int initial_unit_count, float random_start_pos) {
        ...
        for (int i = 0; i < num_players; i++) {
            ...
            for (int k = -(RacesResources.QUARTERS_SIZE /* - 1*/);
                    ...
                    k++) {
                for (int l = -(RacesResources.QUARTERS_SIZE /* - 1*/);
                        l <= (RacesResources.QUARTERS_SIZE /* - 1*/);
                        l++) {
            ...
            for (int k = -(RacesResources.ARMORY_SIZE /* - 1*/);
                    ...
                    k++) {
                for (int l = -(RacesResources.ARMORY_SIZE /* - 1*/);
                        l <= (RacesResources.ARMORY_SIZE /* - 1*/);
                        l++) {
    ...
    public static FogInfo getFogInfo(int terrain_type, int meters_per_world) {
        switch (terrain_type) {
            case NATIVE:
                return new FogInfo(
                        new float[] {.65f, .75f, 1f, 1f},
                        GL11.GL_EXP2,
                        1.3f * meters_per_world,
                        .0015f,
                        0f,
                        meters_per_world >> 2);
            case VIKING:
                return new FogInfo(
                        new float[] {.2f, .4f, .55f, 1f},
                        GL11.GL_EXP2,
                        1.3f * meters_per_world,
                        .0015f,
                        0f,
