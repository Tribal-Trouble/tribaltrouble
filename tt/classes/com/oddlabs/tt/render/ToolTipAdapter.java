...
final strictfp class ToolTipAdapter implements ToolTipVisitor, ToolTip {
    ...
    public final void visitSceneryModel(SceneryModel model) {
        ...
        if (name != null) tool_tip_box.append(name);
    }
                        ...
                        (int)
                                ((watch.length - 1)
                                        * building.getHitPoints()
                                        / building.getBuildingTemplate().getMaxHitPoints())];
    ...
    public final void visitUnit(Unit unit) {
        ...
        if (name != null) tool_tip_box.append(name);
        else tool_tip_box.append(unit.getTemplate().getName());
        ...
        if (unit.getAbilities().hasAbilities(Abilities.MAGIC)) {
            ...
            int index =
                    (int)
                            ((watch.length - 1)
                                    * hit_points
                                    / unit.getUnitTemplate().getMaxHitPoints());
            assert hit_points > 0 && hit_points <= unit.getUnitTemplate().getMaxHitPoints()
                    : "Invalid hit points";
            icon[0] = watch[index];
            tool_tip_box.append(icon);
