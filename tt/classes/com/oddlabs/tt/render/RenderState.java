...
public final strictfp class RenderState implements ElementVisitor {
        ...
        this.default_shadow_renderer =
                (SelectableShadowRenderer)
                        render_queues.getShadowRenderer(
                                render_queues.registerSelectableShadowList(
                                        RacesResources.DEFAULT_SHADOW_DESC));
    ...
    private static final ModelVisitor unit_visitor =
            new SelectableVisitor() {
                        ...
                        if (supply_container.getNumSupplies() > 0) {
                            SpriteRenderer supply_sprite =
                                    render_state.getRenderer(
                                            supply_container.getSupplySpriteRenderer(
    ...
    private float getVisuallyCorrectHeight(float x_f, float y_f) {
        ...
        return local_player
                .getWorld()
                .getHeightMap()
                .computeInterpolatedHeight(patch_level, x_f, y_f);
    }
    ...
    public final void visitEmitter(final Emitter emitter) {
        if (!picking) emitter_queue.add(emitter);
    }

    public final void visitLightning(final Lightning lightning) {
        if (!picking) lightning_queue.add(lightning);
    }

    public final void visitRespond(final LandscapeTargetRespond respond) {
        if (!picking) target_respond_renderer.addToTargetList(respond);
    }

    private static final ModelVisitor supply_model_visitor =
    ...
    private static final ModelVisitor rubber_model_visitor =
            new SupplyModelVisitor() {
                public final void transform(ElementRenderState render_state) {
                    ...
                    RenderTools.translateAndRotate(
                            model.getPositionX(),
                            model.getPositionY(),
                            render_state.f,
                            model.getDirectionX(),
    ...
    public final void visitRubberSupply(final RubberSupply model) {
        float z_offset =
                getVisuallyCorrectHeight(model.getPositionX(), model.getPositionY())
                        + model.getOffsetZ();
    ...
    private static final ModelVisitor scenery_model_visitor =
    ...
    private static final ModelVisitor plants_model_visitor =
            new WhiteModelVisitor() {
                ...
                public final void transform(ElementRenderState render_state) {
                    ...
                    if (dist_squared > START_FADE_DIST * START_FADE_DIST) {
                        ...
                        float alpha =
                                1f
                                        - ((camera_dist - START_FADE_DIST)
                                                / (PLANTS_CUT_DIST - START_FADE_DIST));
    ...
    private static final ModelVisitor directed_weapon_model_visitor =
            new WhiteModelVisitor() {
                public final void transform(ElementRenderState render_state) {
    ...
    private static final ModelVisitor rotating_weapon_model_visitor =
            new WhiteModelVisitor() {
                public final void transform(ElementRenderState render_state) {
