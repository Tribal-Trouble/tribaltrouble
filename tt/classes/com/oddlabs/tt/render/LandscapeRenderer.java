...
public final strictfp class LandscapeRenderer implements Animated {
    ...
    public LandscapeRenderer(
            World world, WorldInfo world_info, GUIRoot gui_root, AnimationManager manager) {
        ...
        this.landscape_vertices =
                new LandscapeTileVertices(
                        world.getHeightMap(),
                        HeightMap.GRID_UNITS_PER_PATCH_EXP,
                        world.getHeightMap().getPatchesPerWorld());
        this.patch_levels =
                new PatchLevel[world.getHeightMap().getPatchesPerWorld()]
                        [world.getHeightMap().getPatchesPerWorld()];
        for (int y = 0; y < patch_levels.length; y++)
            for (int x = 0; x < patch_levels.length; x++) patch_levels[y][x] = new PatchLevel();
        for (int y = 0; y < patch_levels.length; y++)
    ...
    this.structures = world_info.structures;
        ...
        for (int i = 0; i < patch_lists.length; i++) patch_lists[i] = new ArrayList();
        manager.registerAnimation(this);
        this.shadow_indices_buffer =
                BufferUtils.createShortBuffer(
                        LandscapeTileIndices.getNumTriangles(
                                        world.getLandscapeIndices().getNumLOD() - 1)
                                * 3);
        resetEditing();
    }
    ...
    private void renderPatch(LandscapeLeaf leaf) {
        ...
        int patch_index =
                world.getLandscapeIndices()
                        .getPatchIndex(patch_level.getLevel(), patch_level.getBorderSet());
    ...
    private void doPrepareAll(
            final CameraState camera, final boolean visible_override, final Collection result) {
        endEdit();
    ...
    public final void endEdit() {
        ...
        for (int y = edit_patch_y0; y <= edit_patch_y1; y++)
            for (int x = edit_patch_x0; x <= edit_patch_x1; x++) reload(x, y);
        resetEditing();
    }
    ...
    private int calculateLevel(LandscapeLeaf leaf) {
        ...
        float dist2 =
                RenderTools.getEyeDistanceSquared(
                        leaf, camera.getCurrentX(), camera.getCurrentY(), camera.getCurrentZ());
        // Find appropriate patch size
        int i;
        float[] errors = leaf.getErrors();
        for (i = 0; i < errors.length; i++) if (dist2 >= errors[i]) break;
        return i;
    }

    private final PatchGroupVisitor level_updater =
            new PatchGroupVisitor() {
    ...
    private final void setupColormap(int map_x, int map_y) {
        if (current_map_x != map_x || current_map_y != map_y) {
            bindMap(map_x, map_y);
            float tex_translate_x =
                    map_x * world.getHeightMap().getMetersPerChunk()
                            - world.getHeightMap().getMetersPerChunkBorder();
            float tex_translate_y =
                    map_y * world.getHeightMap().getMetersPerChunk()
                            - world.getHeightMap().getMetersPerChunkBorder();
            GLUtils.setupTexGen(
                    world.getHeightMap().getChunkTexScale(),
                    world.getHeightMap().getChunkTexScale(),
                    -tex_translate_x,
                    -tex_translate_y);
    ...
    private final void setupLandscape() {
        ...
        if (Globals.draw_detail) {
            ...
            GL11.glEnable(GL11.GL_TEXTURE_GEN_T);
            GLUtils.setupTexGen(
                    Globals.LANDSCAPE_DETAIL_REPEAT_RATE,
                    Globals.LANDSCAPE_DETAIL_REPEAT_RATE,
                    0,
                    0);
    ...
    private static final void disableLandscape() {
    ...
    final void renderShadow(
            int patch_x, int patch_y, int start_x, int start_y, int end_x, int end_y) {
        landscape_vertices.bind(patch_x, patch_y);
        PatchLevel patch_level = getPatchLevel(patch_x, patch_y);
        shadow_indices_buffer.clear();
        world.getLandscapeIndices()
                .fillCoverIndices(
                        shadow_indices_buffer,
                        patch_level.getLevel(),
                        patch_level.getBorderSet(),
                        start_x,
                        start_y,
                        end_x,
                        end_y);
    ...
    private static final strictfp class Visitor implements PatchGroupVisitor {
        ...
        public final void visitGroup(PatchGroup group) {
            ...
            if (visible_override
                    || (frustum_state = RenderTools.inFrustum(group, camera.getFrustum()))
        ...
        public final void visitLeaf(LandscapeLeaf leaf) {
            if (visible_override
                    || RenderTools.inFrustum(leaf, camera.getFrustum()) >= RenderTools.IN_FRUSTUM) {
                result.add(leaf);
            }
        }
    }
