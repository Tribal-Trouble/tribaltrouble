...
strictfp class TreePicker implements TreeNodeVisitor {
    ...
    private boolean pickingInFrustum(TreeSupply tree_supply, float[][] frustum) {
        picking_selection_box.setBounds(
                ...
                tree_supply.bmin_z,
                tree_supply.bmin_z
                        + (tree_supply.bmax_z - tree_supply.bmin_z)
                                * getHeightScale(tree_supply.getTreeTypeIndex()));
    ...
    private void addToRenderList(TreeSupply tree, CameraState camera) {
        if (isPicking()) markDetailPolygon(tree, SpriteRenderer.HIGH_POLY);
        else sprite_sorter.add(getRenderState(tree), camera, false);
    }
    ...
    public final void visitTree(TreeSupply tree_supply) {
        if (tree_supply.isHidden()) return;
        boolean in_view;
        if (isPicking())
            in_view =
                    !tree_supply.isDead()
                            && (visible_override
                                    || pickingInFrustum(tree_supply, camera.getFrustum()));
        else
            in_view =
                    visible_override
                            || RenderTools.inFrustum(tree_supply, camera.getFrustum())
                                    >= RenderTools.IN_FRUSTUM;
        if (in_view) {
