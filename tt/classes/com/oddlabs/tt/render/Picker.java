...
public final strictfp class Picker implements Updatable {
    ...
    private final Target getNearestPick(List pick_list, Class filter) {
        ...
        for (int i = 0; i < pick_list.size(); i++) {
            ...
            float squared_distance =
                    RenderTools.getCameraDistanceSquared(
                            ((BoundingBox) pickable),
                            tmp_camera.getCurrentX(),
                            tmp_camera.getCurrentY(),
                            tmp_camera.getCurrentZ());
    ...
    public final void pickTarget(
        ...
        if (nearest_pickable != null) {
            if (!(nearest_pickable instanceof SceneryModel)
                    || ((SceneryModel) nearest_pickable).isOccupying())
                respond_manager.addResponder(nearest_pickable);
            if (isNewSetTarget(
                    selection, nearest_pickable, action, Settings.getSettings().aggressive_units))
                player_interface.setTarget(
                        selection,
                        nearest_pickable,
                        action,
            ...
            if (supply != null) {
                ...
                supply.changeRespondingTrees(1);
                if (isNewSetTarget(
                        selection, supply, action, Settings.getSettings().aggressive_units))
                    player_interface.setTarget(
                            selection, supply, action, Settings.getSettings().aggressive_units);
                ...
                if (isNewLandscapeTarget(
                        selection, grid_x, grid_y, action, Settings.getSettings().aggressive_units))
                    player_interface.setLandscapeTarget(
                            selection,
                            grid_x,
                            grid_y,
                            action,
    ...
    private final boolean isNewSetTarget(
            Selectable[] selection, Target target, int action, boolean aggressive) {
    ...
    private final boolean isNewLandscapeTarget(
    ...
    public final Selectable[] pickBoxed(
            CameraState camera, int x1, int y1, int x2, int y2, int clicks) {
        float cx = (x1 + x2) * 0.5f;
        float cy = (y1 + y2) * 0.5f;
        ...
        pickObjects();
        if (StrictMath.abs(x1 - x2) < SELECTION_THRESHOLD
                && StrictMath.abs(y1 - y2) < SELECTION_THRESHOLD)
            return createSinglePick(camera, clicks);
        else return createBoxedPick();
    }
    ...
    private final Selectable[] createBoxedPick() {
        ...
        for (int i = 0; i < element_pick_list.size(); i++) {
            ...
            if (pickable instanceof Selectable) selectables.add(pickable);
        }
    ...
    private static final float computeTMax(float bmin, float bmax, float c, float d) {
        if (d == 0) {
    ...
    private static final float computeTMin(float bmin, float bmax, float c, float d) {
        if (d == 0) {
    ...
    private final boolean doNearestLandscape(
        ...
        while (!patch_pick_set.isEmpty()) {
            ...
            if (t_min < 0) t_min = 0;
            // If t_min is greater than t_max, the pick ray does not intersect the BB, therefore we
            // skip it
            if (t_min >= t_max) {
                continue;
            }
            ...
            if (t_min_height >= 0.001f + t_min_z) {
                //				System.out.println(t_min_x + " " + t_min_y + " " + t_min_height + " " +
                // t_min_z);
    ...
    public final void pickMapGoto(int x, int y, MapCamera camera) {
        setupPicking(camera.getState(), x, y, PICK_SIZE, PICK_SIZE);
        if (nearestLandscape(x, y)) camera.mapGoto(patch_hit_x, patch_hit_y);
    }
    ...
    public final boolean pickLocation(CameraState camera, LandscapeLocation landscape_location) {
        ...
        setupPicking(camera, x, y, PICK_SIZE, PICK_SIZE);
        if (!nearestLandscape(x, y)) return false;
    ...
    private final strictfp class LandscapeLeafComparator implements Comparator {
        private int compare(CameraState camera_state, LandscapeLeaf l1, LandscapeLeaf l2) {
            float l1_dist =
                    RenderTools.getCameraDistanceXYSquared(
                            l1, camera_state.getCurrentX(), camera_state.getCurrentY());
            float l2_dist =
                    RenderTools.getCameraDistanceXYSquared(
                            l2, camera_state.getCurrentX(), camera_state.getCurrentY());
            if (l1_dist < l2_dist) return -1;
            else if (l1_dist > l2_dist) return 1;
            else if (l1.bmin_x < l2.bmin_x) return -1;
            else if (l1.bmin_x > l2.bmin_x) return 1;
            else if (l1.bmin_y < l2.bmin_y) return -1;
            else if (l1.bmin_y > l2.bmin_y) return 1;
            else {
                assert l1 == l2;
                return 0;
