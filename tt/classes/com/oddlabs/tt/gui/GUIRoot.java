...
public final strictfp class GUIRoot extends GUIObject implements Updatable {
    ...
    private final com.oddlabs.tt.resource.Cursor[] cursors =
            new com.oddlabs.tt.resource.Cursor[] {
                new com.oddlabs.tt.resource.Cursor(
                        Utils.makeURL("/textures/gui/pointer_16_1.image"),
                        1,
                        15,
                        Utils.makeURL("/textures/gui/pointer_32_1.image"),
                        2,
                        29,
                        Utils.makeURL("/textures/gui/pointer_32_8.image"),
                        2,
                        29),
                new com.oddlabs.tt.resource.Cursor(
                        Utils.makeURL("/textures/gui/pointer_target_16_1.image"),
                        7,
                        8,
                        Utils.makeURL("/textures/gui/pointer_target_32_1.image"),
                        14,
                        17,
                        Utils.makeURL("/textures/gui/pointer_target_32_8.image"),
                        14,
                        17),
                new com.oddlabs.tt.resource.Cursor(
                        Utils.makeURL("/textures/gui/pointer_text_16_1.image"),
                        4,
                        8,
                        Utils.makeURL("/textures/gui/pointer_text_32_1.image"),
                        6,
                        20,
                        Utils.makeURL("/textures/gui/pointer_text_32_8.image"),
                        6,
                        20)
            };
    ...
    public final void setToolTipTimer() {
        tool_tip_timer.setTimerInterval(
                Settings.getSettings().tooltip_delay * ToolTipBox.MAX_DELAY_SECONDS);
    }
    ...
    public final CameraDelegate getDelegate() {
        if (delegate_stack.size() == 0) return null;
        else return (CameraDelegate) delegate_stack.get(delegate_stack.size() - 1);
    }
    ...
    private final void popModalDelegate(ModalDelegate delegate) {
        ...
        if (top_most && delegate != null) super.addChild(delegate);

        GUIObject object = (GUIObject) focus_backup_stack.remove(index);
        if (delegate_stack.size() > 0) getDelegate().setFocus();
        if (top_most && object != null) object.setFocus();
        mousePick();
    }
    ...
    public static final float getUnitsPerPixel(float z) {
        return (float)
                (z
                        * StrictMath.tan(Globals.FOV * (StrictMath.PI / 180.0f) * 0.5f)
                        / (LocalInput.getViewHeight() * 0.5d));
    }
    ...
    protected final void keyPressed(KeyboardEvent event) {
                ...
                if (event.isControlDown()) {
                    ...
                    info_printer.print(
                            com.oddlabs.tt.util.Utils.getBundleString(
                                    bundle, "screenshot_message", new Object[] {filename}));
                }
                break;
            ...
            case Keyboard.KEY_A:
                if (event.isControlDown()) {
                    ...
                    if (Settings.getSettings().aggressive_units)
                        info_printer.print(
                                com.oddlabs.tt.util.Utils.getBundleString(
                                        bundle, "aggressive_unites_on"));
                    else
                        info_printer.print(
                                com.oddlabs.tt.util.Utils.getBundleString(
                                        bundle, "aggressive_unites_off"));
                }
                break;
        ...
        switch (event.getKeyCode()) {
            case Keyboard.KEY_U:
                //				new AudioPlayer(0f, 0f, 0f, (Audio)Resources.findResource(new
                // AudioFile("/sfx/hit1.ogg")), AudioPlayer.AUDIO_RANK_NOTIFICATION,
                // AudioPlayer.AUDIO_DISTANCE_NOTIFICATION, 1f, 1f, 2f, false, true);
                Renderer.getRenderer().startMovieRecording();
                break;
            case Keyboard.KEY_W:
                if (event.isControlDown()) Globals.draw_water = !Globals.draw_water;
                break;
            case Keyboard.KEY_R:
                if (event.isControlDown()) {
            ...
            case Keyboard.KEY_O:
                ...
                break;
            case Keyboard.KEY_P:
                if (event.isControlDown()) GLUtils.takeScreenshot("");
                else Globals.draw_plants = !Globals.draw_plants;
                break;
            //			 case Keyboard.KEY_H:
            ...
            case Keyboard.KEY_A:
                ...
                break;
            case Keyboard.KEY_M:
                if (event.isControlDown()) Globals.draw_misc = !Globals.draw_misc;
                else {
            ...
            */
            ...
            public Lightning(Vector3f src, Vector3f dst, float width,
            		...
            		AnimationManager manager) {
            */
            //		new Lightning(new Vector3f(src_x, src_y, src_z), new Vector3f(dst_x, dst_y, dst_z),
            // 1f,
    ...
    private void mousePick(int x, int y) {
        ...
        if (target != null && target != current_gui_object) {
            ...
            if (old_tip && !new_tip) {
                if (!render_tool_tip) tool_tip_timer.stop();
                else render_tool_tip = false;
            }
            current_gui_object = target;
            current_gui_object.mouseEnteredAll();
            if (!current_gui_object.isDisabled()) cursor_object = current_gui_object;
        }
    }
    ...
    public final void setupGUIView() {
        ...
        GL11.glLoadIdentity();
        myGLUPerspective(
                Globals.FOV, LocalInput.getViewAspect(), Globals.VIEW_MIN, Globals.VIEW_MAX);
    ...
    public final void myGLUPerspective(double fovy, double aspect, double zNear, double zFar) {
        // From earlier fork by meekee7
        // https://github.com/meekee7/tribaltrouble/blob/master/tt/classes/com/oddlabs/tt/gui/GUIRoot.java
    ...
    final void renderToolTip(ToolTip hovered) {
        if (hovered != null) {
            ...
            tool_tip.render(
                    LocalInput.getMouseX(),
                    LocalInput.getMouseY() - CURSOR_OFFSET_Y,
                    0,
                    getWidth(),
                    0,
                    getHeight());
