...
public final strictfp class InputState {
    ...
    public final void mousePressed(int button) {
        ...
        if (mouse_timer != null) mouse_timer.stop();
        mouse_timer =
                new TimerAnimation(
                        new Updatable() {
                            public final void update(Object anim) {
                                ...
                                if (press_obj == gui_root.getCurrentGUIObject()) {
                                    int local_x =
                                            press_obj.translateXToLocal(LocalInput.getMouseX());
                                    int local_y =
                                            press_obj.translateYToLocal(LocalInput.getMouseY());
    ...
    public final void mouseReleased(int button) {
        ...
        int local_y = gui_hit.translateYToLocal(LocalInput.getMouseY());
        Index.resetBlinking();
        if (button == drag_button) drag_obj = null;
    ...
    public final void mouseDragged(int button, short x, short y) {
        if (drag_obj != null)
            drag_obj.mouseDraggedAll(
                    button, x, y, x - drag_x, y - drag_y, x - absolute_drag_x, y - absolute_drag_y);
        drag_x = x;
        drag_y = y;
    }

    private final boolean clickedSameArea() {
        return StrictMath.abs(LocalInput.getMouseX() - clicked_x) < DOUBLE_CLICK_THRESHOLD
                && StrictMath.abs(LocalInput.getMouseY() - clicked_y) < DOUBLE_CLICK_THRESHOLD;
    }
    ...
    public final void keyTyped(int key_code, char key_char) {
        ...
        KeyboardEvent event =
                new KeyboardEvent(
                        key_code,
                        key_char,
                        LocalInput.isShiftDownCurrently(),
                        LocalInput.isControlDownCurrently());
