...
public final strictfp class BuildSpinner extends IconSpinner {
    ...
    public final int computeCount() {
        if (!current_building.isDead()) {
            ...
            int count =
                    StrictMath.min(
                            build_container.getMaxSupplyCount(),
    ...
    public final void appendToolTip(ToolTipBox tool_tip_box) {
        if (!isDisabled()) super.appendToolTip(tool_tip_box);
    }

    protected final float getProgress() {
        if (!current_building.isDead())
            return ((BuildProductionContainer) current_building.getBuildSupplyContainer(type))
                    .getBuildProgress();
        else return 0;
    }
    ...
    protected void postRender() {
        if (renderInfinite()) Icons.getIcons().getInfinite().render(0, 0);
    }
}
