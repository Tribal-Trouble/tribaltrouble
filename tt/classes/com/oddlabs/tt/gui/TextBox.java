...
public strictfp class TextBox extends TextField implements Scrollable {
    ...
    public TextBox(int width, int height, Font font, int max_chars) {
        ...
        text_renderer =
                new TextBoxRenderer(
                        font,
                        width
                                - edit_box.getLeftOffset()
                                - edit_box.getRightOffset()
                                - scroll_bar.getWidth(),
    ...
    protected void renderGeometry() {
        ...
        text_renderer.render(
                edit_box.getLeftOffset(), edit_box.getBottomOffset(), offset_y, getText());
    }

    protected final void mouseScrolled(int amount) {
        if (amount > 0) setOffsetY(offset_y - 3 * getFont().getHeight());
        else setOffsetY(offset_y + 3 * getFont().getHeight());
    }
    ...
    public final void setOffsetY(int new_offset) {
        ...
        Box edit_box = Skin.getSkin().getEditBox();
        int max_offset_y =
                text_renderer.getTextHeight(getText())
                        - (getHeight() - edit_box.getBottomOffset() - edit_box.getTopOffset());
    ...
    public final void jumpPage(boolean up) {
        ...
        if (up) setOffsetY(offset_y - inner_height);
        else setOffsetY(offset_y + inner_height);
    }
