...
public abstract strictfp class GUIObject extends Renderable {
    ...
    public int translateXToLocal(int x) {
        ...
        if (parent == null) return x - getX();
        else return parent.translateXToLocal(x) - getX();
    }

    public int translateYToLocal(int y) {
        GUIObject parent = (GUIObject) getParent();
        if (parent == null) return y - getY();
        else return parent.translateYToLocal(y) - getY();
    }
    ...
    private final int getXFromDirection(
            int direction, int spacing, int neightbour_x, int neighbour_width) {
        switch (direction) {
            case BOTTOM_LEFT:
            case TOP_LEFT:
                return neightbour_x;
    ...
    private final int getYFromDirection(
            int direction, int spacing, int neighbour_y, int neighbour_height) {
        switch (direction) {
            case BOTTOM_LEFT:
            case BOTTOM_MID:
            case BOTTOM_RIGHT:
    ...
    private void setGlobalFocus() {
        ...
        if (gui_root != null) gui_root.setGlobalFocus(this);
    }

    private GUIObject getGlobalFocus() {
        GUIRoot gui_root = getParentGUIRoot();
        if (gui_root != null) return gui_root.getGlobalFocus();
        else return null;
    }
    ...
    public final boolean isDisabled() {
        GUIObject parent = (GUIObject) getParent();
        if (parent != null) return disabled || parent.isDisabled();
    ...
    public final void removeChild(Renderable child) {
        ...
        if (child == focused_child) {
            ...
            current = (GUIObject) child;
            while (current.getFocusedChild() != null) current = current.getFocusedChild();
            if (current.isFocused()) setGlobalFocus();
        }
    ...
    private final void switchFocusToFirstChild(int dir) {
        ...
        while (gui_child != null) {
            if (gui_child.canFocus()
                    && (min_obj == null
                            || dir * gui_child.getTabOrder() < dir * min_obj.getTabOrder()))
                min_obj = gui_child;
            gui_child = (GUIObject) gui_child.getNext();
        }
        if (min_obj != null) switchFocusToObject(min_obj, dir);
        else if (!focus_cycle) ((GUIObject) getParent()).switchFocusToNextChild(dir);
    }
        ...
        while (gui_child != null) {
            if (gui_child.canFocus()) {
                if (min_obj == null || dir * gui_child.getTabOrder() < dir * min_obj.getTabOrder())
                    min_obj = gui_child;
                if (dir * gui_child.getTabOrder() > dir * tab_order
                        && (greater_obj == null
                                || dir * gui_child.getTabOrder() < dir * greater_obj.getTabOrder()))
    ...
    public final void defocusBranch() {
        if (getFocusedChild() != null) getFocusedChild().defocusBranch();
    ...
    protected final void focusNotifyAll(boolean focus) {
        ...
        for (int i = 0; i < focus_listeners.size(); i++) {
            ...
            if (listener != null) listener.activated(focus);
        }
    }
    ...
    public final void mouseScrolledAll(int amount) {
        ...
        for (int i = 0; i < mouse_wheel_listeners.size(); i++) {
            ...
            if (listener != null) listener.mouseScrolled(amount);
        }
    }

    protected void mouseScrolled(int amount) {
        GUIObject parent = (GUIObject) getParent();
        if (parent != null) parent.mouseScrolledAll(amount);
    }

    public final void mouseDraggedAll(
            int button,
            int x,
            int y,
            int relative_x,
    ...
    public final void mouseMovedAll(int x, int y) {
        ...
        for (int i = 0; i < mouse_motion_listeners.size(); i++) {
            MouseMotionListener listener = (MouseMotionListener) mouse_motion_listeners.get(i);
            if (listener != null) listener.mouseMoved(x, y);
        }
    }

    protected void mouseMoved(int x, int y) {
        GUIObject parent = (GUIObject) getParent();
        if (parent != null) parent.mouseMovedAll(x, y);
    }
    ...
    public final void mouseExitedAll() {
        ...
        for (int i = 0; i < mouse_motion_listeners.size(); i++) {
            MouseMotionListener listener = (MouseMotionListener) mouse_motion_listeners.get(i);
            if (listener != null) listener.mouseExited();
        }
    }
    ...
    public final void mouseEnteredAll() {
        ...
        for (int i = 0; i < mouse_motion_listeners.size(); i++) {
            MouseMotionListener listener = (MouseMotionListener) mouse_motion_listeners.get(i);
            if (listener != null) listener.mouseEntered();
        }
    }
    ...
    public final void mouseClickedAll(int button, int x, int y, int clicks) {
        ...
        for (int i = 0; i < mouse_click_listeners.size(); i++) {
            ...
            if (listener != null) listener.mouseClicked(button, x, y, clicks);
        }
    }

    protected void mouseClicked(int button, int x, int y, int clicks) {
        GUIObject parent = (GUIObject) getParent();
        if (parent != null) parent.mouseClickedAll(button, x, y, clicks);
    }
    ...
    public final void mouseReleasedAll(int button, int x, int y) {
        ...
        for (int i = 0; i < mouse_button_listeners.size(); i++) {
            ...
            if (listener != null) listener.mouseReleased(button, x, y);
        }
    }

    protected void mouseReleased(int button, int x, int y) {
        GUIObject parent = (GUIObject) getParent();
        if (parent != null) parent.mouseReleasedAll(button, x, y);
    }
    ...
    public final void mousePressedAll(int button, int x, int y) {
        ...
        for (int i = 0; i < mouse_button_listeners.size(); i++) {
            MouseButtonListener listener = (MouseButtonListener) mouse_button_listeners.get(i);
            if (listener != null) listener.mousePressed(button, x, y);
        }
    }

    protected void mousePressed(int button, int x, int y) {
        GUIObject parent = (GUIObject) getParent();
        if (parent != null) parent.mousePressedAll(button, x, y);
    }
    ...
    public final void mouseHeldAll(int button, int x, int y) {
        ...
        for (int i = 0; i < mouse_button_listeners.size(); i++) {
            MouseButtonListener listener = (MouseButtonListener) mouse_button_listeners.get(i);
            if (listener != null) listener.mouseHeld(button, x, y);
        }
    }

    protected void mouseHeld(int button, int x, int y) {
        GUIObject parent = (GUIObject) getParent();
        if (parent != null) parent.mouseHeldAll(button, x, y);
    }
