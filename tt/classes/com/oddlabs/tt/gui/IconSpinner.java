...
public abstract strictfp class IconSpinner extends GUIObject implements ToolTip {
    ...
    private final void addDemoForm() {
        ...
        Form demo_form =
                new InGameDemoForm(
                        viewer,
                        Utils.getBundleString(db, "chicken_weapons_unavailable_header"),
                        new GUIImage(512, 256, 0f, 0f, 1f, 1f, "/textures/gui/demo_chickenweapons"),
                        Utils.getBundleString(db, "chicken_weapons_unavailable"));
    ...
    public final void shortcutPressed(boolean shift_down, boolean ctrl_down) {
        if (!isDisabled()) {
            if (showNag()) {
                ...
                int mouse_button;
                if (ctrl_down) mouse_button = LocalInput.RIGHT_BUTTON;
                else mouse_button = LocalInput.LEFT_BUTTON;

                if (shift_down) button_minus.mousePressedAll(mouse_button, 0, 0);
                else button_plus.mousePressedAll(mouse_button, 0, 0);
            }
        }
    }
    ...
    protected final void renderGeometry() {
        ...
        if (isDisabled()) icon_quad[Skin.DISABLED].render(x, y);
        ...
        if (text_count > 0) {
            ...
            watch[index].render(
                    getWidth() - watch[index].getWidth(), getHeight() - watch[index].getHeight());
        }
    }
    ...
    private final strictfp class IncreaseListener implements MouseButtonListener {
        ...
        public final void mousePressed(int button, int x, int y) {
            if (showNag()) {
                ...
                if (button == LocalInput.RIGHT_BUTTON) increase(10);
                else increase(1);
            }
        }
    ...
    private final strictfp class DecreaseListener implements MouseButtonListener {
        ...
        public final void mousePressed(int button, int x, int y) {
            if (showNag()) {
                ...
                if (button == LocalInput.RIGHT_BUTTON) decrease(10);
                else decrease(1);
            }
        }
