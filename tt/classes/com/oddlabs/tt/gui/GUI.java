...
public final strictfp class GUI implements Animated {
    ...
    final void switchRoot(GUIRoot gui_root, UIRenderer renderer) {
        if (current_root != null) current_root.removeTree();
    ...
    public final void render(AmbientAudio ambient, CameraState frustum_state) {
        ...
        if (clear_color) GL11.glClear(GL11.GL_COLOR_BUFFER_BIT | GL11.GL_DEPTH_BUFFER_BIT);
        else GL11.glClear(GL11.GL_DEPTH_BUFFER_BIT);
        if (renderer != null) renderer.render(ambient, frustum_state, current_root);
        renderGUI();
    }
    ...
    public final void pickHover() {
        ...
        if (renderer != null)
            renderer.pickHover(
                    gui_hit.canHoverBehind(),
                    camera,
                    LocalInput.getMouseX(),
                    LocalInput.getMouseY());
    }
    ...
    public final void renderGUI() {
        ...
        if (current_root.showToolTip()) {
            ...
            if (tooltip == null && renderer != null) tooltip = renderer.getToolTip();
            if (tooltip != null) current_root.renderToolTip(tooltip);
        }
        if (renderer != null) renderer.renderGUI(current_root);
        current_root.renderTopmost();
        if (fade != null) fade.render();
