...
public strictfp class ChatPanel extends Panel implements ChatListener {
    ...
    private static final String getI18N(String key) {
    ...
    public ChatPanel(
            ...
            int compare_height,
            int button_width,
            EnterListener chat_listener,
            MouseClickListener leave_listener) {
        super(getI18N("chat"));
        ...
        int edit_line_height =
                edata.getBottomOffset()
                        + edata.getTopOffset()
                        + Skin.getSkin().getEditFont().getHeight();
        int height =
                compare_height
                        - pdata.getTopOffset()
                        - pdata.getBottomOffset()
                        - edit_line_height
                        - label_headline.getHeight()
                        - 2 * fdata.getObjectSpacing();
        int user_list_height =
                (height - Skin.getSkin().getFormData().getObjectSpacing())
                        / 2; // - Skin.getSkin().getEditFont().getHeight();
        user_list_width =
                2 * button_width
                        + 2 * fdata.getObjectSpacing()
                        - Skin.getSkin().getScrollBarData().getScrollBar().getWidth();

        ColumnInfo[] lobby_infos =
                new ColumnInfo[] {new ColumnInfo(getI18N("lobby"), user_list_width)};
        lobby_users_list_box =
                new MultiColumnComboBox(gui_root, lobby_infos, user_list_height, true);
        addChild(lobby_users_list_box);

        ColumnInfo[] playing_infos =
                new ColumnInfo[] {new ColumnInfo(getI18N("playing"), user_list_width)};
        playing_users_list_box =
                new MultiColumnComboBox(gui_root, playing_infos, user_list_height, true);
        ...
        int width =
                compare_width
                        - pdata.getLeftOffset()
                        - pdata.getRightOffset()
                        - lobby_users_list_box.getWidth();
    ...
    public final void update(ChatRoomInfo info) {
        ...
        if (users != null) {
            ...
            for (int i = 0; i < users.length; i++) {
                ...
                if (!users[i].isPlaying()) lobby_users_list_box.addRow(row);
                else playing_users_list_box.addRow(row);
            }
        }
        refreshMessages();
    }
    ...
    public final void connectionLost() {
        if (private_message_form != null) private_message_form.remove();
    }
    ...
    private final strictfp class ClearListener implements EnterListener {
