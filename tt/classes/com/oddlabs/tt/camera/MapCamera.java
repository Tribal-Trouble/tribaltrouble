...
public final strictfp class MapCamera extends Camera {
    ...
    private final Label label =
            new Label(
                    Utils.getBundleString(
                            ResourceBundle.getBundle(MapCamera.class.getName()), "map_mode"),
    ...
    public final void doAnimate(float t) {
        float factor =
                t
                        * 1000f
                        / StrictMath.max(
                                t * 1000f, Settings.getSettings().mapmode_delay * MAP_TIME_FACTOR);
        float dx;
        float dy;
        float dz;
        float da;
    ...
    private final void changeMode(int mode) {
        ...
        if (mode == IN_MAP) {
            label.setPos(
                    (LocalInput.getViewWidth() - label.getWidth()) / 2,
                    LocalInput.getViewHeight() - label.getHeight());
