...
public final strictfp class InGameChatForm extends Form implements ChatListener {
    ...
    public InGameChatForm(InfoPrinter info_printer, WorldViewer viewer) {
        ...
        HorizButton button_send =
                new HorizButton(Utils.getBundleString(bundle, "send"), BUTTON_WIDTH);
        addChild(button_send);
        button_send.addMouseClickListener(new SendListener());

        chat_box =
                new TextBox(
                        CHAT_WIDTH + BUTTON_WIDTH, CHAT_HEIGHT, Skin.getSkin().getEditFont(), -1);
        addChild(chat_box);
        ...
        radio_all =
                new RadioButton(
                        true, radio_button_group, Utils.getBundleString(bundle, "send_to_all"));
        addChild(radio_all);

        radio_team =
                new RadioButton(
                        false, radio_button_group, Utils.getBundleString(bundle, "send_to_team"));
        addChild(radio_team);
    ...
    public final void setReceivers(boolean all) {
        if (all) radio_button_group.mark(radio_all);
        else radio_button_group.mark(radio_team);
    }
