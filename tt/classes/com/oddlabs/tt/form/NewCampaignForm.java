...
public final strictfp class NewCampaignForm extends Form
        implements DeterministicSerializerLoopbackInterface {
    ...
    public NewCampaignForm(
            NetworkSelector network, GUIRoot gui_root, Menu main_menu, CampaignForm campaign_form) {
        ...
        Label label_headline =
                new Label(
                        Utils.getBundleString(bundle, "caption"), Skin.getSkin().getHeadlineFont());
        addChild(label_headline);

        // name
        Group group = new Group();
        Label name_label =
                new Label(Utils.getBundleString(bundle, "name"), Skin.getSkin().getEditFont());
        ...
        // race
        Label race_label =
                new Label(Utils.getBundleString(bundle, "race"), Skin.getSkin().getEditFont());
        ...
        Label difficulty_label =
                new Label(
                        Utils.getBundleString(bundle, "difficulty"), Skin.getSkin().getEditFont());
    ...
    private final boolean nameIsUnique(String name) {
        if (campaign_states != null) {
            for (int i = 0; i < campaign_states.length; i++) {
                if (campaign_states[i].getName().equals(name)) return false;
            }
        }
        return true;
    }
    ...
    private final strictfp class RaceListener implements ItemChosenListener {
        public final void itemChosen(PulldownMenu menu, int item_index) {
            if (item_index == INDEX_NATIVES
                    && (!Settings.getSettings().has_native_campaign || !Renderer.isRegistered())) {
                menu.chooseItem(INDEX_VIKINGS);
                gui_root.addModalForm(
                        new MessageForm(Utils.getBundleString(bundle, "native_unavailable")));
            }
        }
    }
}
