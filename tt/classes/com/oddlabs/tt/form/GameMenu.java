...
public final strictfp class GameMenu extends Panel implements ConfigurationListener, ChatListener {
    ...
    public GameMenu(
        ...
        chat_info =
                new Label(
                        Utils.getBundleString(bundle, "chat"), Skin.getSkin().getEditFont(), width);
        ...
        HorizButton info_button =
                new HorizButton(Utils.getBundleString(bundle, "info"), button_width);
        ...
        if (rated) {
            Label rating =
                    new Label(
                            Utils.getBundleString(bundle, "rating"),
                            font,
                            RATING_WIDTH,
                            Label.ALIGN_RIGHT);
            addChild(rating);
                ...
                if ((player.getType() != PlayerSlot.OPEN && player.getType() != PlayerSlot.HUMAN)
                    ...
                    if (player_slot == local_player_slot) {
                        ...
                        game_network
                                .getClient()
                                .getServerInterface()
                                .resetSlotState(player_slot, true);
                    }
                }
                break;
            case CLOSED_INDEX:
                if (player.getType() != PlayerSlot.CLOSED || race_changed || team_changed) {
                    ...
                    game_network
                            .getClient()
                            .getServerInterface()
                            .resetSlotState(player_slot, false);
                }
                break;
    ...
    private final int countHumans(PlayerSlot[] players) {
        ...
        for (int i = 0; i < players.length; i++)
            if (players[i].getType() == PlayerSlot.HUMAN) result++;
        return result;
    }
    ...
    public final void setPlayers(PlayerSlot[] players) {
        ...
        for (int i = 0; i < players.length; i++) {
            ...
            race_button
                    .getMenu()
                    .chooseItem(player.getInfo() != null ? player.getInfo().getRace() : 0);
            team_button
                    .getMenu()
                    .chooseItem(player.getInfo() != null ? player.getInfo().getTeam() : 0);
                ...
                switch (player.getType()) {
                    ...
                    case PlayerSlot.HUMAN:
                        ...
                        slot_button
                                .getMenu()
                                .getItem(OPEN_INDEX)
                                .setLabelString(NickUtils.toDisplayName(player_name));
                        slot_button.getMenu().chooseItem(OPEN_INDEX);
    ...
    private final void updateRatedLabels(int[] player_slots, int[] player_ratings, int[][] points) {
        ...
        for (int i = 0; i < player_slots.length; i++) {
            ...
            if (slot == local_player_slot) {
                ...
                String rating_change_message =
                        Utils.getBundleString(
                                bundle,
                                "rating_change_message",
                                new Object[] {Integer.toString(win), Integer.toString(-lose)});
    ...
    private final GUIObject createPlayerPulldown(
        ...
        PulldownItem computer_easy_item =
        ...
        PulldownItem computer_hard_item =
                new PulldownItem(Utils.getBundleString(bundle, "hard_ai"));
        ...
        PulldownButton pulldown_button =
                new PulldownButton(gui_root, pulldown_menu, CLOSED_INDEX, 150);
        ...
        PulldownButton race_pulldown_button =
                new PulldownButton(gui_root, race_pulldown_menu, 0, 115);
    ...
    private final void playerLeft(String name) {
        ...
        chat_box.append(
                Utils.getBundleString(
                        bundle, "left_game", new Object[] {NickUtils.toDisplayName(name)}));
        finishChatAppend();
    }
    ...
    private final void playerJoined(String name) {
        ...
        chat_box.append(
                Utils.getBundleString(
                        bundle, "joined_game", new Object[] {NickUtils.toDisplayName(name)}));
        finishChatAppend();
    }
    ...
    private static int getNumTeams(PlayerSlot[] players) {
        ...
        for (int i = 0; i < players.length; i++) {
            ...
            if (current.getInfo() != null) teams.add(new Integer(current.getInfo().getTeam()));
        }
        return teams.size();
    }
    ...
    private final strictfp class StartListener implements MouseClickListener {
        public final void mouseClicked(int button, int x, int y, int clicks) {
            ...
            if (num_teams < MIN_TEAMS) {
                String err_msg =
                        Utils.getBundleString(
                                bundle, "min_teams", new Object[] {Integer.toString(MIN_TEAMS)});
    ...
    private final strictfp class SendListener implements MouseClickListener {
        public final void mouseClicked(int button, int x, int y, int clicks) {
