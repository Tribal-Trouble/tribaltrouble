...
public final strictfp class SelectGameMenu extends Form
        implements MatchmakingListener, TerrainMenuListener {
    ...
    public SelectGameMenu(
            NetworkSelector network, GUIRoot gui_root, Menu main_menu, int panel_index) {
        ...
        Label label_headline =
                new Label(
                        Utils.getBundleString(bundle, "multiplayer_caption"),
        ...
        game_list_pulldown_menu.addItem(
                new PulldownItem(Utils.getBundleString(bundle, "game_info")));
        ...
        HorizButton update_list_button =
                new HorizButton(
                        Utils.getBundleString(bundle, "update_list"), BUTTON_WIDTH_EXTRA_LONG);
        game_list_panel.addChild(update_list_button);
        update_list_button.addMouseClickListener(new UpdateGameListListener());

        HorizButton create_button =
                new HorizButton(Utils.getBundleString(bundle, "create_game"), BUTTON_WIDTH_LONG);
        game_list_panel.addChild(create_button);
        create_button.addMouseClickListener(new CreateGameListener());

        HorizButton join_button =
                new HorizButton(Utils.getBundleString(bundle, "join_game"), BUTTON_WIDTH);
        ...
        label_headline =
                new Label(
                        Utils.getBundleString(bundle, "league_description"),
                        Skin.getSkin().getHeadlineFont());
        highscore_list_panel.addChild(label_headline);
        user_name_size = 250;
        ColumnInfo[] score_infos =
                new ColumnInfo[] {
        ...
        HorizButton update_scores_button =
                new HorizButton(
                        Utils.getBundleString(bundle, "update_scores"), BUTTON_WIDTH_EXTRA_LONG);
        ...
        label_headline =
                new Label(
                        Utils.getBundleString(bundle, "chat_rooms_caption"),
                        Skin.getSkin().getHeadlineFont());
        ...
        update_list_button =
                new HorizButton(
                        Utils.getBundleString(bundle, "update_rooms"), BUTTON_WIDTH_EXTRA_LONG);
        chat_room_list_panel.addChild(update_list_button);
        update_list_button.addMouseClickListener(new UpdateRoomListListener());

        create_button =
                new HorizButton(Utils.getBundleString(bundle, "create_room"), BUTTON_WIDTH_LONG);
        ...
        HorizButton logout_button =
                new HorizButton(Utils.getBundleString(bundle, "logout"), BUTTON_WIDTH);
        ...
        if (Network.getMatchmakingClient().getProfile() == null && Renderer.isRegistered()) {
            ...
            if (Network.getMatchmakingClient().getProfile() == null && !Renderer.isRegistered()) {
                ...
                Form demo_form =
                        new DemoForm(
                                gui_root,
                                Utils.getBundleString(db, "profile_unavailable_header"),
                                new GUIImage(
                                        512, 256, 0f, 0f, 1f, 1f, "/textures/gui/demo_multiplayer"),
                                Utils.getBundleString(db, "profile_unavailable"));
    ...
    private final ChatPanel createChatRoomPanel(ChatRoomInfo info) {
        ChatPanel panel =
                new ChatPanel(
                        gui_root,
                        info,
                        chat_room_list_panel.getWidth(),
                        chat_room_list_panel.getHeight(),
                        BUTTON_WIDTH_SHORT,
                        new SendChatListener(),
                        new LeaveListener());
    ...
    public final void createGameMenu(
            GameNetwork game_network, Game game, WorldGenerator generator, int player_slot, int player_count) {
        game_panel =
                new GameMenu(
                        game_network,
                        gui_root,
    ...
    private static final void updateList(int type) {
    ...
    public final void connectionLost() {
        leaveChatRoom();
        remove();
        if (profiles_form != null) profiles_form.connectionLost();
    ...
    public final void receivedList(int type, Object[] names) {
        switch (type) {
            case MatchmakingServerInterface.TYPE_GAME:
                for (int i = 0; i < names.length; i++) game_hosts.add(names[i]);
                updateGameListGUI();
                break;
            case MatchmakingServerInterface.TYPE_CHAT_ROOM_LIST:
                for (int i = 0; i < names.length; i++) chat_rooms.add(names[i]);
                updateChatRoomListGUI();
                break;
            case MatchmakingServerInterface.TYPE_RANKING_LIST:
                for (int i = 0; i < names.length; i++) updateRankingList((RankingEntry) names[i]);
                break;
            default:
            ...
            Row row =
                    new Row(
                            new GUIObject[] {
                                ...
                                new Label(
                                        ServerMessageBundler.getGamespeedString(
                                                game_host.getGame().getGamespeed()),
                                        combofont),
    ...
    private final void updateChatRoomListGUI() {
        ...
        for (int i = 0; i < chat_rooms.size(); i++) {
            ...
            String users_and_max =
                    Utils.getBundleString(
                            bundle,
                            "users_and_max",
                            new Object[] {
                                new Integer(chat_room_info.getNumJoined()),
                ...
                if (rated
                    ...
                    String min_wins =
                            Utils.getBundleString(
                                    bundle,
                                    "min_wins",
                                    new Object[] {new Integer(GameSession.MIN_WINS_FOR_RANKING)});
                    ...
                    main_menu.joinGame(
                            network,
                            gui_root.getGUI(),
                            selected_game.getHostID(),
                            game.isRated(),
                            game.getGamespeed(),
                            game.getMapcode(),
                            this,
                            game.getRandomStartPos(),
    ...
    private final strictfp class GameListPanelListener implements FocusListener {
        public final void activated(boolean activated) {
            if (activated) updateList(MatchmakingServerInterface.TYPE_GAME);
        }
    }
    ...
    private final strictfp class CreateRoomListener implements MouseClickListener {
        public final void mouseClicked(int button, int x, int y, int clicks) {
            if (Network.getMatchmakingClient().getProfile() != null) {
                main_menu.setMenuCentered(
                        new CreateChatRoomForm(gui_root, main_menu, SelectGameMenu.this));
            }
        }
    }

    private final strictfp class SendChatListener implements EnterListener {
    ...
    private final strictfp class LeaveListener implements MouseClickListener {
        public final void mouseClicked(int button, int x, int y, int clicks) {
            leaveChatRoom();
        }
    }
