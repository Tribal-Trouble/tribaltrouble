...
public final strictfp class RouterClient implements ConnectionInterface {
    private final ARMIInterfaceMethods interface_methods =
            new ARMIInterfaceMethods(RouterClientInterface.class);
    ...
    public RouterClient(NetworkSelector network, RouterHandler router_handler, int port) {
        ...
        this.connection =
                new Connection(
                        network, new InetSocketAddress(Utils.getLoopbackAddress(), port), this);
        this.game_interface =
                (GameInterface) ARMIEvent.createProxy(connection, GameInterface.class);
    }
