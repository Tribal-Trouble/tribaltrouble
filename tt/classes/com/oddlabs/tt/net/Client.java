...
public final strictfp class Client
        implements ARMIEventBroker, GameClientInterface, ConnectionInterface {
    ...
    private final ARMIInterfaceMethods interface_methods =
            new ARMIInterfaceMethods(GameClientInterface.class);
    ...
    public Client(
        ...
        if (host_id != -1) this.connection = new TunnelledConnection(host_id, this);
        else
            this.connection =
                    new Connection(
                            network,
                            new InetSocketAddress(Utils.getLoopbackAddress(), Globals.NET_PORT),
                            this);
        gameserver_interface =
                (GameServerInterface) ARMIEvent.createProxy(connection, GameServerInterface.class);

        this.unit_infos = new UnitInfo[MatchmakingServerInterface.MAX_PLAYERS];
        for (int i = 0; i < unit_infos.length; i++)
            unit_infos[i] =
                    new UnitInfo(false, false, 0, false, Player.INITIAL_UNIT_COUNT, 0, 0, 0);
    }
    ...
    public final void chat(int player_slot, String chat) {
        if (chat != null && player_slot >= 0 && player_slot < player_slots.length)
            Network.getChatHub()
                    .chat(
                            new ChatMessage(
                                    player_slots[player_slot].getInfo().getName(),
                                    chat,
                                    ChatMessage.CHAT_GAME_MENU));
    }
    ...
    public final void close() {
        connection.close();
        state = CLOSED;
        if (cleanup_action != null) cleanup_action.run();
    }
