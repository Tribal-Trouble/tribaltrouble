...
public final strictfp class Server implements ConnectionListenerInterface {
    ...
    private final void unregisterGame() {
        ...
        if (tunnelled_listener != null) tunnelled_listener.close();
    ...
    private final int getNumReady() {
        ...
        while (it.hasNext()) {
            ...
            if (client.getClient().getPlayerSlot().isReady()) count++;
        }
        return count;
    }
    ...
    private final ClientConnection locateClientForSlot(PlayerSlot player_slot) {
        ...
        while (it.hasNext()) {
            ClientConnection client = (ClientConnection) it.next();
            if (client.getClient().getPlayerSlot() == player_slot) return client;
        }
        return null;
    }

    public final void resetSlotState(PlayerSlot client_slot, int slot, boolean open) {
        if (!canControlSlot(client_slot, slot)) return;
    ...
    private void resetSlotState(PlayerSlot client_slot, boolean open) {
        ...
        if (player_client != null) disconnectClient(player_client);
    ...
    public final void startServer(PlayerSlot slot) {
        if (!canControlSlot(slot, 0)
                || getNumReady() != getNumClients()) // || PlayerSlot.getNumTeams(players) < 2)
        return;
        state = SYNCHRONIZING;
        unregisterGame();
        broadcastInits();
    ...
    public final void setPlayerSlot(
            PlayerSlot client_slot,
            int slot,
            int type,
            int race,
            int team,
            boolean ready,
            int ai_difficulty) {
        if (!PlayerSlot.isValidType(type) || !RacesResources.isValidRace(race)) return;
        if (!canControlSlot(client_slot, slot)
                || (client_slot.getSlot() == slot && type != PlayerSlot.HUMAN)) return;
        ...
        if (player_client != null && type != PlayerSlot.HUMAN) disconnectClient(player_client);
        String name;
        ...
        boolean reset_ready =
                player_slot.getInfo() == null
                        || type != player_slot.getType()
                        || ai_difficulty != player_slot.getAIDifficulty()
                        || !player_info.equals(player_slot.getInfo());
    ...
    private final void resetReady() {
        ...
        for (int i = 0; i < players.length; i++) {
            ...
            if (player_slot.getType() == PlayerSlot.HUMAN) num_humans++;
        }
        if (num_humans > 1) {
            for (int i = 0; i < players.length; i++) {
                PlayerSlot player_slot = players[i];
                if (player_slot.getType() == PlayerSlot.HUMAN) player_slot.setReady(false);
            }
        }
    }

    private final void broadcastPlayers(boolean reset_ready) {
        if (reset_ready) resetReady();
        Iterator it = getClientIterator();
        while (it.hasNext()) {
            ClientConnection client = (ClientConnection) it.next();
    ...
    private final short locateAvailableSlot() {
        for (short i = 0; i < players.length; i++) {
            if (players[i].getType() == PlayerSlot.OPEN) return i;
        }
        return (short) -1;
    }
    ...
    public final void incomingConnection(
        ...
        if (state != NEGOTIATING
                || available_slot == -1
                || (remote_address instanceof InetAddress
                        && !((InetAddress) remote_address).isLoopbackAddress())
                || (remote_address instanceof TunnelIdentifier
                        && game != null
                        && game.isRated()
                        && ((TunnelIdentifier) remote_address).getProfile().getWins()
                                < GameSession.MIN_WINS_FOR_RANKING)) {
            System.out.println(
                    "rejecting incoming connection since state = "
                            + state
                            + " | locateAvailableSlot() = "
                            + available_slot
                            + " remote_address = "
            ...
            if (profile != null) {
                ...
                name =
                        Utils.getBundleString(
                                ResourceBundle.getBundle(MatchmakingClient.class.getName()),
                                "player");
        } else {
