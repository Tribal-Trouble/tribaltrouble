...
public final strictfp class WorldViewer implements Animated {
    ...
    public WorldViewer(
        ...
        AudioImplementation audio_impl =
                new AudioImplementation() {
                    ...
                    public final void playerGamespeedChanged() {
                        ...
                        for (int i = 0; i < players.length; i++) {
                            ...
                            if (World.isValidGamespeed(preferred_gamespeed)) {
                                ...
                                count++;
                                result +=
                                        players[i].getPlayerInfo().getName()
                                                + ": "
                                                + ServerMessageBundler.getGamespeedString(
                                                        preferred_gamespeed);
                            }
                        }
                        if (count > 0 && isMultiplayer()) gui_root.getInfoPrinter().print(result);
                    }

                    public final void newAttackNotification(Selectable target) {
                        Player owner = target.getOwner();
                        if (owner == getLocalPlayer())
                            notification_manager.newAttackNotification(
                                    animation_manager_local, target, getLocalPlayer());
                    }

                    public final void newSelectableNotification(Selectable target) {
                        Player owner = target.getOwner();
                        if (owner == getLocalPlayer())
                            notification_manager.newSelectableNotification(
                                    target, animation_manager_local, getLocalPlayer());
                    }
        ...
        WorldInfo world_info =
                generator.generate(
                        player_slots.length,
                        world_params.getInitialUnitCount(),
    ...
    private void initPlayer(
        ...
        if (slot.getType() == PlayerSlot.AI) {
            ...
            for (int j = 0; j < unit_info.getNumPeons(); j++, i++) {
                new Unit(
                        player,
                        starting_location[2 * i],
                        starting_location[2 * i + 1],
                        null,
                        player.getRace().getUnitTemplate(Race.UNIT_PEON));
            }
                ...
                if (player.getRace().getChieftainAI() instanceof VikingChieftainAI)
                    chieftain =
                            new Unit(
                                    ...
                                    starting_location[2 * i + 1],
                                    null,
                                    player.getRace().getUnitTemplate(Race.UNIT_CHIEFTAIN),
                                    Utils.getBundleString(bundle, "chieftain_name"),
                                    false);
                    ...
                    chieftain =
                            new Unit(
                                    ...
                                    null,
                                    player.getRace().getUnitTemplate(Race.UNIT_CHIEFTAIN),
                                    Utils.getBundleString(bundle, "native_chieftain_name"),
                                    false);
