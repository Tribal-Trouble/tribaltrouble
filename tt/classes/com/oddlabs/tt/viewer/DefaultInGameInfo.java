...
public strictfp class DefaultInGameInfo implements InGameInfo {
    private final ResourceBundle terrain_menu_bundle =
            ResourceBundle.getBundle(TerrainMenu.class.getName());
    ...
    public void addGameOverGUI(
            WorldViewer viewer, GameStatsDelegate delegate, int header_y, Group group) {
        addGameOverGUI(viewer, delegate, header_y, group, true);
    }

    protected final void addGameOverGUI(
        ...
        map_code.setPos(
                (delegate.getWidth() - map_code.getWidth()) / 2, header_y - map_code.getHeight());

        HorizButton button_replay =
                new HorizButton(
                        Utils.getBundleString(GameStatsDelegate.bundle, "replay_island"), 150);
        button_replay.addMouseClickListener(
        ...
        HorizButton button_observer =
                new HorizButton(
                        Utils.getBundleString(GameStatsDelegate.bundle, "observer_mode"), 150);
        ...
        HorizButton button_end =
                new HorizButton(Utils.getBundleString(GameStatsDelegate.bundle, "main_menu"), 150);
        ...
        for (int i = 0; i < players.length; i++) {
            ...
            String team_str =
                    Utils.getBundleString(
                            terrain_menu_bundle,
                            "team",
                            new Object[] {Integer.toString(player_info.getTeam() + 1)});
            Label team = new Label(team_str, Skin.getSkin().getHeadlineFont());
            team.setColor(color);
            names.addChild(name);
            if (last_name != null) name.place(last_name, GUIObject.BOTTOM_LEFT);
            else name.place();
            last_name = name;
        ...
        viewer.getGUIRoot()
                .pushDelegate(
                        new GameStatsDelegate(
                                viewer,
                                viewer.getGUIRoot().getDelegate().getCamera(),
                                Utils.getBundleString(Menu.bundle, "game_aborted")));
    }
