...
public final strictfp class PathTracker {
    ...
    public static final int OK_INTERRUPTIBLE = 2;
    public static final int DONE = 3;
    public static final int SOFTBLOCKED = 4;
    ...
    private final void doAnimate(float speed) {
        ...
        update();
        if (bezier_path.isDone()) state = OK_INTERRUPTIBLE;
        else state = OK;
    }
    ...
    private final PathTracker getNextDeadlocked() {
        ...
        if (occupant != null && occupant != unit && occupant instanceof Movable) {
            ...
            if (next.isMoving()
                    && (next.getTracker().state == SOFTBLOCKED
                            || next.getTracker().state == BLOCKED)) {
    ...
    private final void checkRegionPath(int src_x, int src_y) {
        ...
        if (target_region != null && tracker_algorithm.acceptRegion(target_region)) {
            while (region_path != null) {
                ...
                if (current_region == region_path_region) return;
                region_path = (RegionNode) region_path.getParent();
            }
        }
        target_region = tracker_algorithm.findPathRegion(src_x, src_y);
        if (target_region != null) region_path = (RegionNode) target_region.newPath();
        else region_path = null;
    }

    private final GridPathNode findPathToNextRegion(
            int src_x, int src_y, RegionNode next_region_node, boolean allow_secondary_targets) {
        ...
        return tracker_algorithm.findPathGrid(
                target_region, next_region, src_x, src_y, allow_secondary_targets);
    }
    ...
    private final int lookAhead() {
        ...
        if (occupant != null) {
            ...
            for (int i = 0; i < REGION_SEARCH_TRIES; i++) {
                patch_path =
                        findPathToNextRegion(
                                unit.getGridX(), unit.getGridY(), search_next_region_node, false);
                if (done(unit.getGridX(), unit.getGridY())) return DONE;
                if (patch_path != null || search_next_region_node == null) break;
            ...
            if (occupant != null) {
                ...
                if (occupant.getPenalty() < Occupant.STATIC) return SOFTBLOCKED;
                else return BLOCKED;
            }
        } else if (grid_path == null && !done(next_unit_grid_x, next_unit_grid_y)) {
            grid_path =
                    findPathToNextRegion(
                            next_unit_grid_x, next_unit_grid_y, next_region_node, true);
        }
        return OK;
    }
    ...
    private final int initPath() {
        ...
        RegionNode next_region_node = (RegionNode) region_path.getParent();
        GridPathNode init_path =
                findPathToNextRegion(unit.getGridX(), unit.getGridY(), next_region_node, true);
        if (done(unit.getGridX(), unit.getGridY())) return DONE;
        if (init_path == null) {
            return BLOCKED;
        }
