...
public final strictfp class RegionBuilder {
    ...
    public static final void buildRegions(UnitGrid unit_grid, float start_x_f, float start_y_f) {
        ...
        while ((start_node = findStartNode(unit_grid, region_nodes, start_nodes)) != null) {
            assert !unit_grid.isGridOccupied(start_node.getGridX(), start_node.getGridY())
                    : "Starting location (" + start_x + "," + start_y + ") occupied";
        ...
        for (int y = 0; y < grid_size; y++)
            for (int x = 0; x < grid_size; x++) {
                ...
                if (region != null) updateRegionNeighbours(unit_grid, x, y, region);
            }
    ...
    private static final void testNeighbour(
            UnitGrid unit_grid, int grid_x, int grid_y, Region region) {
    ...
    private static final void addRegionNodes(
        ...
        while (region_nodes.size() > 0) {
            ...
            if (unit_grid.getRegion(node.getGridX(), node.getGridY()) != null) continue;
    ...
    private static final RegionBuilderNode findStartNode(
            UnitGrid unit_grid, PocketList region_nodes, QueueArray start_nodes) {
        region_nodes.clear();
