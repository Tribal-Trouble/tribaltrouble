...
final strictfp class GridNode extends Node {
    ...
    private static final DirectionNode lookupDirectionNode(int dx, int dy) {
    ...
    public static final Offset setupPathFinding(
        ...
        if (StrictMath.abs(dst_grid_x - src_grid_x) >= RegionBuilder.GRID_SIZE
                && StrictMath.abs(dst_grid_y - src_grid_y) >= RegionBuilder.GRID_SIZE) return null;
    ...
    public static final GridNode getPathfinderNode(Offset offset, int x, int y) {
        GridNode node = getPathfinderNodeOffset(x - offset.offset_x, y - offset.offset_y);
        if (node != null) node.offset = offset;
        return node;
    }

    private static final GridNode getPathfinderNodeOffset(int local_x, int local_y) {
        if (local_x < 0
                || local_x >= pathfinder_grid.length
                || local_y < 0
                || local_y >= pathfinder_grid.length) return null;
        else return pathfinder_grid[local_y][local_x];
    }

    private final boolean addNeighbour(
        ...
        if (occupant != null) {
            ...
            if (penalty >= Occupant.STATIC) return false;
            cost += penalty;
        }
    ...
    static final strictfp class Offset {
        final int offset_x;
        final int offset_y;
