...
public final strictfp class HeightMap {
    ...
    private static final StrictVector3f vec1 = new StrictVector3f();
    private static final StrictVector3f vec2 = new StrictVector3f();
    private static final StrictVector3f plane = new StrictVector3f();
    ...
    public HeightMap(
        ...
        int texels_per_colormap_noborder =
                texels_per_colormap - 2 * Globals.TEXELS_PER_CHUNK_BORDER * chunks_per_colormap;
    ...
    public final boolean isGridInside(int x, int y) {
        boolean inside_world =
                x >= 0 && y >= 0 && x < getGridUnitsPerWorld() && y < getGridUnitsPerWorld();
        return inside_world;
    }

    public final boolean isInside(float x, float y) {
        boolean inside_world =
                x >= 0 && y >= 0 && x <= getMetersPerWorld() && y <= getMetersPerWorld();
        return inside_world;
    }
    ...
    final void makePlaneVector(
            int x0, int y0, int x1, int y1, int x2, int y2, StrictVector3f plane) {
        makePlaneVector(
                x0,
                y0,
                getWrappedHeight(x0, y0),
                x1,
                y1,
                getWrappedHeight(x1, y1),
                x2,
                y2,
                getWrappedHeight(x2, y2),
                plane);
    }

    private static final void makePlaneVector(
            float h1x,
            float h1y,
            float h1z,
            float h2x,
    ...
    private static final float doPlane(
            float x,
            float y,
            float h1x,
            float h1y,
    ...
    public final boolean isBelowSeaLevel(int patch_x, int patch_y) {
        ...
        for (int y = 0; y < getGridUnitsPerPatch(); y++) {
            for (int x = 0; x < getGridUnitsPerPatch(); x++) {
                ...
                if (height < getSeaLevelMeters()) return true;
            }
        }
        return false;
    }
    ...
    public final float getNearestHeight(float x_f, float y_f) {
        float y_z =
                computeInterpolatedHeight(
                        world_instance.getLandscapeIndices().getNumLOD() - 1, x_f, y_f);
        return y_z;
    }

    public final float getClampedHeight(int grid_x, int grid_y) {
        if (grid_x < 0 || grid_x >= world.length) grid_x = 0;
        if (grid_y < 0 || grid_y >= world.length) grid_y = 0;
    ...
    public final void editHeight(int grid_x, int grid_y, float height) {
        ...
        int patch_x0 =
                (patch_x1 - (x_border ? 1 : 0) + patches_per_world) & (patches_per_world - 1);
        int patch_y0 =
                (patch_y1 - (y_border ? 1 : 0) + patches_per_world) & (patches_per_world - 1);

        for (int y = patch_y0; y <= patch_y1; y++)
            for (int x = patch_x0; x <= patch_x1; x++) landscape_leaves[y][x].editHeight(height);
        world_instance
                .getNotificationListener()
                .patchesEdited(patch_x0, patch_y0, patch_x1, patch_y1);
    }
}
