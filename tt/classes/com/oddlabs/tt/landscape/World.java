...
public final strictfp class World {
    ...
    private static final float[] GAMESPEEDS =
            new float[] {
                0f,
    ...
    public static final World newWorld(
            AudioImplementation audio_implementation,
            LandscapeResources landscape_resources,
            RacesResources races_resources,
            LowDetailModel[] tree_low_details,
    ...
    if (game_mode != null && game_mode.peaceEnabled && game_mode.peaceSeconds > 0) {
        ...
        world =
                new HeightMap(
                        this,
                        world_info.meters_per_world,
                        world_info.sea_level_meters,
        ...
        System.out.println(
                "****************** Finished landscape in "
                        + ((time_stop - time_start) / 1000f)
                        + " sec ********************");
        this.supply_managers = new SupplyManagers(this);
        this.unit_grid = new UnitGrid(world);
        RegionBuilder.buildRegions(
                unit_grid,
                world_info.starting_locations[0][0],
                world_info.starting_locations[0][1]);
        this.landscape_indices =
                new LandscapeTileIndices(world, HeightMap.GRID_UNITS_PER_PATCH_EXP);
        this.patch_root = new PatchGroup(this);
        this.tree_root =
                AbstractTreeGroup.newRoot(
                        this,
                        tree_low_details,
                        world_info.trees,
        ...
        AbstractElementNode.buildSupplies(
                this, world_info.iron, world_info.rocks, world_info.plants, terrain_type);
    }
