...
public final strictfp class IslandGenerator implements WorldGenerator {
    ...
    private final Texture createDetail(GLImage detail_image, int base_level) {
        ...
        GLImage.updateMipMapsArea(
                detail_mipmaps,
                base_level,
                Globals.LANDSCAPE_DETAIL_FADEOUT_FACTOR,
                0,
                0,
                detail_mipmaps[0].getWidth(),
                detail_mipmaps[0].getHeight(),
                false);
        return new Texture(
                detail_mipmaps,
    ...
    private final int getTexelsPerGridUnit() {
        int texels_per_grid_unit =
                Globals.TEXELS_PER_GRID_UNIT
                        / (int)
                                StrictMath.pow(
                                        2,
    ...
    public final WorldInfo generate(
            int num_players, int initial_unit_count, float random_start_pos) {
        ...
        if (detail_prefade_level < 0) detail_prefade_level = 0;
        float detail_prefade =
                IDEAL_DETAIL_ALPHA
                        * (float)
                                StrictMath.pow(
                                        Globals.LANDSCAPE_DETAIL_FADEOUT_FACTOR,
        ...
        do {
            chunk_maps =
                    blendTextures(
                            ...
                            blend_infos,
                            alpha_size,
                            Globals.STRUCTURE_SIZE,
                            colormap_size / alpha_size);
        } while (chunk_maps == null);
        ProgressForm.progress();
        return new WorldInfo(
                meters_per_world,
                landscape.getSeaLevelMeters(),
                colormap_size,
                chunks_per_colormap,
                chunk_maps,
                detail,
    ...
    private static final Texture[][] blendTextures(
            OffscreenRendererFactory factory,
            int chunks_per_colormap,
            BlendInfo[] blend_infos,
            int alpha_size,
            int structure_size,
            int scale) {
        ...
        float structure_texels_per_chunk_border =
                Globals.TEXELS_PER_CHUNK_BORDER * structure_texel_size;
        float structure_offset =
                TEXELS_PER_CHUNK * structure_texel_size - 2 * structure_texels_per_chunk_border;
            ...
            for (int x = 0; x < chunk_maps[y].length; x++) {
                ...
                while (mip_size >= 1) {
                    ...
                    for (int i = 0; i < blend_infos.length; i++) {
                        blend_infos[i].setup();
                        float structure_offset_u =
                                x * structure_offset - structure_texels_per_chunk_border;
                        float structure_offset_v =
                                y * structure_offset - structure_texels_per_chunk_border;
                        ...
                        structure_tex_coords
                                .put(2, structure_offset_u + structure_length)
                                .put(3, structure_offset_v);
                        structure_tex_coords
                                .put(4, structure_offset_u + structure_length)
                                .put(5, structure_offset_v + structure_length);
                        structure_tex_coords
                                .put(6, structure_offset_u)
                                .put(7, structure_offset_v + structure_length);
                        alpha_tex_coords.put(0, alpha_offset_u).put(1, alpha_offset_v);
                        alpha_tex_coords
                                .put(2, alpha_offset_u + alpha_length)
                                .put(3, alpha_offset_v);
                        alpha_tex_coords
                                .put(4, alpha_offset_u + alpha_length)
                                .put(5, alpha_offset_v + alpha_length);
                        alpha_tex_coords
                                .put(6, alpha_offset_u)
                                .put(7, alpha_offset_v + alpha_length);
