...
public final strictfp class AdvancedAI extends AI {
    ...
    private final void nodeDefend(int score) {
        ...
        if (result > 0) {
            ...
            getOwner()
                    .setLandscapeTarget(
                            units,
                            defense_target.getGridX(),
                            defense_target.getGridY(),
                            Target.ACTION_DEFEND,
                            true);
        }
    }
    ...
    private final void nodeGuardTowers(int num_towers) {
        if ((getTowers() == null && num_towers > 0)
                || (getTowers() != null && num_towers > getTowers().length)) {
            nodeBuildTower(num_towers);
        } else if (num_towers > 0) {
            for (int i = 0; i < getTowers().length; i++) {
                if (!((Building) getTowers()[i]).getUnitContainer().isSupplyFull()
                        && getIdleWarriors() != null
    ...
    private final void nodeBuildTower(int number) {
        if (!towerUnderConstruction()
                && ((getTowers() == null && number == 1)
                        || (getTowers() != null && getTowers().length < number))
                && getQuarters() != null
                && getArmory() != null) {
            ...
            Building origin;
            if (number % 2 == 1) origin = (Building) getQuarters()[0];
            else origin = (Building) getArmory()[0];
    ...
    private final void nodeAssignIdlePeons() {
        if (getIdlePeons() != null) {
            if (quartersUnderConstruction() && getConstructionSites() != null) {
                ...
                getOwner()
                        .setTarget(getIdlePeons(), getQuarters()[0], Target.ACTION_DEFAULT, false);
            }
        }
    }
    ...
    private final void nodeAttackWithWarriorsAndChieftain(int num_warriors, boolean use_chieftain) {
        ...
        */
        if (getIdleWarriors() != null
                && getIdleWarriors().length >= num_warriors
            ...
            if (use_chieftain) nodeTrainChieftain();
        }
    }
            ...
            if (!armory.isDead()) {
                ...
                if (num_units >= num_warriors && num_weapons >= num_warriors) {
                    int num_rubber_units =
                            StrictMath.min(
                                    num_warriors,
                                    armory.getSupplyContainer(RubberAxeWeapon.class)
                                            .getNumSupplies());
                    int num_iron_units =
                            StrictMath.min(
                                    num_warriors - num_rubber_units,
                                    armory.getSupplyContainer(IronAxeWeapon.class)
                                            .getNumSupplies());
                    int num_rock_units =
                            StrictMath.min(
                                    num_warriors - num_rubber_units - num_iron_units,
                                    armory.getSupplyContainer(RockAxeWeapon.class)
                                            .getNumSupplies());
                    if (num_rubber_units > 0)
                        getOwner()
                                .deployUnits(
                                        armory,
                                        Building.KEY_DEPLOY_RUBBER_WARRIOR,
                                        num_rubber_units);
                    if (num_iron_units > 0)
                        getOwner()
                                .deployUnits(
                                        armory, Building.KEY_DEPLOY_IRON_WARRIOR, num_iron_units);
                    if (num_rock_units > 0)
                        getOwner()
                                .deployUnits(
                                        armory, Building.KEY_DEPLOY_ROCK_WARRIOR, num_rock_units);
                } else {
    ...
    private final void nodeGather(Building armory, int num_units) {
        ...
        if (getGatherTreePeons() != null) tree = getGatherTreePeons().length;
        if (getGatherRockPeons() != null) rock = getGatherRockPeons().length;
        if (getGatherIronPeons() != null) iron = getGatherIronPeons().length;
        if (getGatherRubberPeons() != null) rubber = getGatherRubberPeons().length;

        if (tree >= MAX_UNITS_GATHERING_TREE[difficulty]) tree = Integer.MAX_VALUE;
        if (rock >= MAX_UNITS_GATHERING_ROCK[difficulty]) rock = Integer.MAX_VALUE;
        if (iron >= MAX_UNITS_GATHERING_IRON[difficulty]) iron = Integer.MAX_VALUE;
        if (rubber >= MAX_UNITS_GATHERING_RUBBER[difficulty]) rubber = Integer.MAX_VALUE;

        boolean deployed;
        do {
            deployed = false;
            if (num_units > 0
                    && tree < MAX_UNITS_GATHERING_TREE[difficulty]
                    && tree <= rock
                    && tree <= iron
                    && tree <= rubber) {
                ...
                tree++;
                    ...
                    && rock < MAX_UNITS_GATHERING_ROCK[difficulty]
                    && rock <= tree
                    && rock <= iron
                    && rock <= rubber) {
                ...
                rock++;
                    ...
                    && iron < MAX_UNITS_GATHERING_IRON[difficulty]
                    && iron <= tree
                    && iron <= rock
                    && iron <= rubber) {
                ...
                iron++;
                    ...
                    && rubber < MAX_UNITS_GATHERING_RUBBER[difficulty]
                    && rubber <= tree
                    && rubber <= rock
                    && rubber <= iron) {
    ...
    private final void nodeTransferUnits(int num_units, Building armory) {
        ...
        if (quarters != null) {
            if (!quarters.isDead()) {
                ...
                if (quarters.getUnitContainer().getNumSupplies()
                        > MIN_UNITS_REPRODUCING[difficulty]) {
                    int units =
                            (int)
                                    StrictMath.min(
                                            num_units,
                                            quarters.getUnitContainer().getNumSupplies()
                                                    - MIN_UNITS_REPRODUCING[difficulty]);
    ...
    private final void nodeBuildArmory() {
        ...
        if (!armoryUnderConstruction()
                && getArmory() == null
                && getQuarters() != null
                && ((Building) getQuarters()[0]).getAbilities().hasAbilities(Abilities.REPRODUCE)) {
            Selectable[] builders = getPeons(20);
            if (builders.length < 20) {
                if (quarters != null
                        && !quarters.isDead()
                        && quarters.getUnitContainer().getNumSupplies() >= 20)
                    getOwner().deployUnits(quarters, Building.KEY_DEPLOY_PEON, 20);
            }
            if (builders.length == 0) return;

            // TODO: Should use Quarters as origin, if it exists
            setArmoryUnderConstruction(
                    buildBuilding(
    ...
    private final Selectable[] getPeons(int min_num_peons) {
        ...
        Selectable[][] peon_types =
                new Selectable[][] {
                    getGatherIronPeons(),
                    getGatherRockPeons(),
                    getGatherTreePeons(),
                    getGatherRubberPeons()
                };
    ...
    */
    ...
    private final Target findTarget(int start_x, int start_y) {
        ...
        int squared_dist_building =
                (best_building.getGridX() - start_x) * (best_building.getGridX() - start_x)
                        + (best_building.getGridY() - start_y)
                                * (best_building.getGridY() - start_y);
        int squared_dist_target =
                (best_target.getGridX() - start_x) * (best_target.getGridX() - start_x)
                        + (best_target.getGridY() - start_y) * (best_target.getGridY() - start_y);

        if (squared_dist_target < squared_dist_building / 2) return best_target;
        else return best_building;
    }

    private final boolean buildBuilding(
            int building_type, Selectable[] selection, int grid_x, int grid_y) {
        BuildingSiteScanFilter filter =
                new BuildingSiteScanFilter(
                        getUnitGrid(),
        ...
        if (target_list.size() > 0) {
            ...
            getOwner()
                    .placeBuilding(selection, building_type, target.getGridX(), target.getGridY());
            return true;
        } else {
            return false;
        }
