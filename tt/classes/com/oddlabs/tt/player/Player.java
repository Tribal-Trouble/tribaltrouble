...
public final strictfp class Player implements PlayerInterface {
    ...
    public final void setPreferredGamespeed(int speed) {
        if (!World.isValidPreferredGamespeed(speed)) return;
    ...
    public final int getGamespeed() {
        if (World.isValidGamespeed(preferred_speed)) return preferred_speed;
        else return world.getGamespeed();
    }
    ...
    public final void buildRockWeapons(Building building, int num_weapons, boolean infinite) {
        if (isValid(building)) building.buildWeapons(RockAxeWeapon.class, num_weapons, infinite);
    }

    public final void buildIronWeapons(Building building, int num_weapons, boolean infinite) {
        if (isValid(building)) building.buildWeapons(IronAxeWeapon.class, num_weapons, infinite);
    }

    public final void buildRubberWeapons(Building building, int num_weapons, boolean infinite) {
        if (isValid(building)) building.buildWeapons(RubberAxeWeapon.class, num_weapons, infinite);
    }

    public final void doMagic(Unit chieftain, int magic) {
        if (isValid(chieftain)) chieftain.doMagic(magic, true);
    }

    public final void exitTower(Building building) {
        if (isValid(building)) building.exitTower();
    }

    public final void trainChieftain(Building building, boolean start) {
        if (isValid(building)) building.trainChieftain(start);
    }
    ...
    public final void setRallyPoint(Building building, Target target) {
        if (isValid(building) && target != null) building.setRallyPoint(target);
    }
    ...
    public final void setTarget(
        ...
        for (int i = 0; i < selection.length; i++) {
            if (isValid(selection[i])) selection[i].initTarget(target, action, aggressive);
        }
    }

    public final void killSelection(Selectable[] selection) {
        for (int i = 0; i < selection.length; i++) {
            if (selection[i] != null) selection[i].hit(10000, 0f, 1f, this);
        }
    }
    ...
    public final boolean teamHasBuilding() {
        Player[] players = world.getPlayers();
        for (int i = 0; i < players.length; i++)
            if (players[i].getPlayerInfo().getTeam() == player_info.getTeam()
                    && players[i].getBuildingCountContainer().getNumSupplies() > 0) return true;
        return false;
    }
