...
public abstract strictfp class AI implements Animated {
        ...
        if (unit_info != null) {
            ...
            for (int i = 0; i < unit_info.getNumPeons(); i++) {
                ...
                new Unit(
                        ...
                        t.getPositionY(),
                        null,
                        owner.getRace().getUnitTemplate(Race.UNIT_PEON));
            }
            for (int i = 0; i < unit_info.getNumRockWarriors(); i++) {
                ...
                new Unit(
                        ...
                        t.getPositionY(),
                        null,
                        owner.getRace().getUnitTemplate(Race.UNIT_WARRIOR_ROCK));
            }
            for (int i = 0; i < unit_info.getNumIronWarriors(); i++) {
                ...
                new Unit(
                        ...
                        t.getPositionY(),
                        null,
                        owner.getRace().getUnitTemplate(Race.UNIT_WARRIOR_IRON));
            }
            for (int i = 0; i < unit_info.getNumRubberWarriors(); i++) {
                ...
                new Unit(
                        ...
                        t.getPositionY(),
                        null,
                        owner.getRace().getUnitTemplate(Race.UNIT_WARRIOR_RUBBER));
            }
        }
    }
    ...
    protected final Selectable[] getIdlePeons() {
        if (INDEX_IDLE_PEONS == -1) return null;
        else return lists[INDEX_IDLE_PEONS];
    }

    protected final Selectable[] getIdleChieftains() {
        if (INDEX_IDLE_CHIEFTAINS == -1) return null;
        else return lists[INDEX_IDLE_CHIEFTAINS];
    }

    protected final Selectable[] getIdleWarriors() {
        if (INDEX_IDLE_WARRIORS == -1) return null;
        else return lists[INDEX_IDLE_WARRIORS];
    }

    protected final Selectable[] getGatherTreePeons() {
        if (INDEX_GATHER_TREE_PEONS == -1) return null;
        else return lists[INDEX_GATHER_TREE_PEONS];
    }

    protected final Selectable[] getGatherRockPeons() {
        if (INDEX_GATHER_ROCK_PEONS == -1) return null;
        else return lists[INDEX_GATHER_ROCK_PEONS];
    }

    protected final Selectable[] getGatherIronPeons() {
        if (INDEX_GATHER_IRON_PEONS == -1) return null;
        else return lists[INDEX_GATHER_IRON_PEONS];
    }

    protected final Selectable[] getGatherRubberPeons() {
        if (INDEX_GATHER_RUBBER_PEONS == -1) return null;
        else return lists[INDEX_GATHER_RUBBER_PEONS];
    }

    protected final Selectable[] getArmory() {
        if (INDEX_ARMORY == -1) return null;
        else return lists[INDEX_ARMORY];
    }

    protected final Selectable[] getQuarters() {
        if (INDEX_QUARTERS == -1) return null;
        else return lists[INDEX_QUARTERS];
    }

    protected final Selectable[] getTowers() {
        if (INDEX_TOWERS == -1) return null;
        else return lists[INDEX_TOWERS];
    }

    protected final Selectable[] getConstructionSites() {
        if (INDEX_CONSTRUCTION_SITES == -1) return null;
        else return lists[INDEX_CONSTRUCTION_SITES];
    }

    protected final Selectable[] getPlaceBuildingPeons() {
        if (INDEX_PLACE_BUILDING_PEONS == -1) return null;
        else return lists[INDEX_PLACE_BUILDING_PEONS];
    }

    protected final Selectable[] getDefendingUnits() {
        if (INDEX_DEFENDING_UNITS == -1) return null;
        else return lists[INDEX_DEFENDING_UNITS];
    }
    ...
    protected final boolean shouldDoAction(float time) {
        sleep_time -= time;
        if (!Globals.run_ai || sleep_time >= 0) return false;
        reset();
        return true;
    }
    ...
    public final void manTowers(int num_towers) {
        ...
        if (towers == null || idle_warriors == null) return;

        int length = (int) StrictMath.min(idle_warriors.length, towers.length);
        for (int i = 0; i < length; i++) {
            owner.setTarget(
                    new Selectable[] {idle_warriors[i]}, towers[i], Target.ACTION_DEFAULT, false);
        }
    }

    public static final int attackLandscape(Player owner, Target target, int num_warriors) {
        int ordered = 0;
        Selectable[][] lists = owner.classifyUnits();
        for (int i = 0; i < lists.length; i++) {
            Selectable s = lists[i][0];
            if (s instanceof Unit
                    && !(s.getPrimaryController() instanceof WalkController
                            && ((WalkController) s.getPrimaryController()).isAgressive())) {
                for (int j = 0; j < lists[i].length; j++) {
                    Unit unit = (Unit) s;
                    if (unit.getAbilities().hasAbilities(Abilities.THROW)) {
                        owner.setLandscapeTarget(
                                new Selectable[] {lists[i][j]},
                                target.getGridX(),
                                target.getGridY(),
    ...
    public static final Unit getWarrior(Player owner) {
        Selectable[][] lists = owner.classifyUnits();
        for (int i = 0; i < lists.length; i++) {
            Selectable s = lists[i][0];
