...
public abstract class Island {
    ...
    protected final GameNetwork startNewGame(
        ...
        WorldInitAction init_action =
                new WorldInitAction() {
                    public final void run(WorldViewer viewer) {
                                ...
                                for (int i = 0; i < players.length; i++)
                                    if (players[i].isEnemy(viewer.getLocalPlayer()))
                                        viewer.getLocalPlayer()
                                                .setHitBonus(CAMPAIGN_DIFFICULTY_BONUS);
                                break;
                        ...
                        start();
                        new DefeatTrigger(
                                world_viewer, campaign, viewer.getLocalPlayer().getChieftain());
                    }
                };
        return Menu.startNewGame(
                network,
                gui_root,
                null,
                new WorldParameters(
    ...
    protected final void placePrisoners(
            Player captive,
            Player enemy,
            int peons,
            int rock_warriors,
        ...
        for (int i = 0; i < peons; i++) {
            new Unit(
                    captive,
                    UnitGrid.coordinateFromGrid(tx),
                    UnitGrid.coordinateFromGrid(ty),
                    null,
                    captive.getRace().getUnitTemplate(Race.UNIT_PEON));
        }
        ...
        if (chieftain) {
            captive.setActiveChieftain(
                    new Unit(
                            captive,
                            UnitGrid.coordinateFromGrid(tx),
                            UnitGrid.coordinateFromGrid(ty),
                            null,
                            captive.getRace().getUnitTemplate(Race.UNIT_CHIEFTAIN)));
        }
    }
    ...
    protected final void refillArmory(Player enemy) {
        if (enemy.getQuarters() == null || enemy.getArmory() == null) return;

        enemy.getQuarters().removeSupplies(Unit.class);
        enemy.getArmory()
                .fillSupplies(
                        Unit.class,
                        enemy.getWorld().getMaxUnitCount()
                                - enemy.getUnitCountContainer().getNumSupplies());
