...
public final strictfp class NativeChieftainAI extends ChieftainAI {
    ...
    private final void nodeLightningCloud(Unit chieftain) {
        if (chieftain.getMagicProgress(RacesResources.INDEX_MAGIC_LIGHTNING) < 1) return;
    ...
    private final void nodePoisonFog(Unit chieftain) {
        if (chieftain.getMagicProgress(RacesResources.INDEX_MAGIC_POISON) < 1) return;

        float hit_radius =
                chieftain
                        .getOwner()
                        .getRace()
                        .getMagicFactory(RacesResources.INDEX_MAGIC_POISON)
                        .getHitRadius();
        int num_enemy_units = numEnemyUnits(chieftain.getOwner());
        int num_enemy_units_close = getNumEnemyUnitsClose(chieftain, hit_radius);
    ...
    private final int getNumEnemyUnitsClose(Unit chieftain, float hit_radius) {
        FindOccupantFilter filter =
                new FindOccupantFilter(
                        chieftain.getPositionX(),
                        chieftain.getPositionY(),
                        hit_radius,
                        chieftain,
                        Unit.class);
        chieftain
        ...
        for (int i = 0; i < target_list.size(); i++) {
            ...
            if (chieftain.getOwner().isEnemy(unit.getOwner())
                    && squared_dist < hit_radius * hit_radius) {
    ...
    private final int getNumFriendlyUnitsClose(Unit chieftain, float hit_radius) {
        ...
        for (int i = 0; i < target_list.size(); i++) {
            ...
            float squared_dist = dx * dx + dy * dy;
            if (!chieftain.getOwner().isEnemy(s.getOwner())
                    && squared_dist < hit_radius * hit_radius) {
