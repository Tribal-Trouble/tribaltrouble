...
public abstract strictfp class InGameDelegate extends CameraDelegate {
    ...
    private boolean cheat(KeyboardEvent event) {
        ...
        if (!cheat.isEnabled()) return false;
        ...
        switch (event.getKeyCode()) {
            case Keyboard.KEY_F1:
                if (viewer.getLocalPlayer().getUnitCountContainer().getNumSupplies()
                        != viewer.getParameters().getMaxUnitCount()) {
                    new Unit(
            ...
            case Keyboard.KEY_F5:
                if (!viewer.getLocalPlayer().hasActiveChieftain()
                        && !viewer.getLocalPlayer().isTrainingChieftain()) {
                    Unit chieftain =
                            new Unit(
                                    viewer.getLocalPlayer(),
                                    landscape_x,
            ...
            case Keyboard.KEY_F6:
                viewer.getLocalPlayer()
                        .killSelection(
                                viewer.getSelection().getCurrentSelection().filter(Abilities.NONE));
                return true;
        ...
        if (!Settings.getSettings().inDeveloperMode()) return false;
                ...
                if (event.isControlDown()) {
                    ...
                    if (set.size() > 0) {
                        ...
                        if (s instanceof Building) {
                            ...
                            if (!building.isDead()
                                    && !building.getAbilities().hasAbilities(Abilities.ATTACK))
                                building.printDebugInfo();
                        } else if (s instanceof Unit) {
                            Unit unit = (Unit) s;
                            if (!unit.isDead()) unit.printDebugInfo();
                        }
                    }
                    return true;
                }
