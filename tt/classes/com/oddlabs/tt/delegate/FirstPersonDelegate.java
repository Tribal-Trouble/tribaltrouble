...
public strictfp class FirstPersonDelegate extends InGameDelegate {
    ...
    public void mouseMoved(int x, int y) {
        if (!done) getCamera().mouseMoved(x, y);
    }
    ...
    public void mouseDragged(
            ...
            int absolute_x,
            int absolute_y) {
        if (created_tick == LocalEventQueue.getQueue().getManager().getTick()) return;
        if ((button == LocalInput.MIDDLE_BUTTON || key_pressed)
                && !done
                && getGUIRoot().getModalDelegate() == null) {
