...
public final strictfp class CampaignMapForm extends CameraDelegate {
    ...
    public CampaignMapForm(
            final NetworkSelector network, final GUIRoot gui_root, Campaign campaign) {
        ...
        if (campaign.getState().getRace() == CampaignState.RACE_VIKINGS) {
            ...
            if (campaign.getState().getCurrentIsland() == 14) {
                ...
                final Runnable runnable_next =
                        new Runnable() {
                            public final void run() {
                                CampaignDialogForm dialog =
                                        new CampaignDialogForm(
                                                Utils.getBundleString(
                                                        bundle, "native_campaign_opened_header"),
                                                Utils.getBundleString(
                                                        bundle, "native_campaign_opened"),
                                                null,
                ...
                CampaignDialogForm dialog =
                        new CampaignDialogForm(
                                Utils.getBundleString(bundle, "viking_header"),
                ...
                if(campaign.getState().getDifficulty() == CampaignState.DIFFICULTY_NORMAL) {
                    ...
                    SteamAchievementManager.getAchievementManager().unlockAchievement(SteamAchievementNames.COMPLETE_VIKING_CAMPAIGN_HARD);
                }
        ...
        if (campaign.getState().getRace() == CampaignState.RACE_NATIVES) {
            ...
            if (campaign.getState().getCurrentIsland() == 7) {
                ...
                CampaignDialogForm dialog =
                        new CampaignDialogForm(
                                Utils.getBundleString(bundle, "native_header"),
                                Utils.getBundleString(bundle, "native_campaign_completed"),
                                campaign.getIcons().getFaces()[0],
                                CampaignDialogForm.ALIGN_IMAGE_LEFT,
                                runnable);
                if(campaign.getState().getDifficulty() == CampaignState.DIFFICULTY_NORMAL) {
                    SteamAchievementManager.getAchievementManager().unlockAchievement(SteamAchievementNames.COMPLETE_NATIVE_CAMPAIGN_NORMAL);
                }
                else if(campaign.getState().getDifficulty() == CampaignState.DIFFICULTY_HARD) {
                    SteamAchievementManager.getAchievementManager().unlockAchievement(SteamAchievementNames.COMPLETE_NATIVE_CAMPAIGN_HARD);
                }
                gui_root.addModalForm(dialog);
            }
        }
        ...
        for (int i = 0; i < campaign.getIcons().getNumIslands(); i++) {
            ...
            if (island != null) island.setPos(data.getX(), data.getY());
        }
    }
    ...
    public static final void closeCampaign(NetworkSelector network, GUI gui) {
