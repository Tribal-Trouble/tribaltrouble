...
public final strictfp class GameStatsDelegate extends CameraDelegate implements Updatable {
    ...
    public static final ResourceBundle bundle =
            ResourceBundle.getBundle(GameStatsDelegate.class.getName());
    ...
    public GameStatsDelegate(WorldViewer viewer, Camera old_camera, String label_str) {
        ...
        addChild(label);
        label.setPos(
                (getWidth() - label.getWidth()) / 2, (getHeight() - label.getHeight()) * 4 / 5);
        ...
        addChild(score_box);
        score_box.setPos(
                (getWidth() - score_box.getWidth()) / 2, (getHeight() - score_box.getHeight()) / 2);

        GUIObject[] units_lost_labels = new GUIObject[players.length + 1];
        units_lost_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "units_lost"),
                        0,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            units_lost_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getUnitsLost(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
        ...
        units_killed_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "units_killed"),
                        1,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            units_killed_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getUnitsKilled(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        score_box.addRow(new Row(units_killed_labels, null));

        GUIObject[] buildings_lost_labels = new GUIObject[players.length + 1];
        buildings_lost_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "buildings_lost"),
                        2,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            buildings_lost_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getBuildingsLost(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        score_box.addRow(new Row(buildings_lost_labels, null));

        GUIObject[] buildings_destroyed_labels = new GUIObject[players.length + 1];
        buildings_destroyed_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "buildings_wrecked"),
                        3,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            buildings_destroyed_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getBuildingsDestroyed(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        score_box.addRow(new Row(buildings_destroyed_labels, null));

        GUIObject[] tree_harvested_labels = new GUIObject[players.length + 1];
        tree_harvested_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "tree_resources"),
                        3,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            tree_harvested_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getTreeHarvested(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        score_box.addRow(new Row(tree_harvested_labels, null));

        GUIObject[] rock_harvested_labels = new GUIObject[players.length + 1];
        rock_harvested_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "rock_resources"),
                        4,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            rock_harvested_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getRockHarvested(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        score_box.addRow(new Row(rock_harvested_labels, null));

        GUIObject[] iron_harvested_labels = new GUIObject[players.length + 1];
        iron_harvested_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "iron_resources"),
                        5,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            iron_harvested_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getIronHarvested(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        score_box.addRow(new Row(iron_harvested_labels, null));

        GUIObject[] rubber_harvested_labels = new GUIObject[players.length + 1];
        rubber_harvested_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "chicken_resources"),
                        6,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            rubber_harvested_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getRubberHarvested(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        ...
        walked_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "meters_walked"),
                        7,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            walked_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getUnitsMoved() * 2,
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        ...
        weapons_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "weapons_thrown"),
                        8,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            weapons_labels[i + 1] =
                    new IntegerLabel(
                            players[i].getWeaponsThrown(),
                            Skin.getSkin().getMultiColumnComboBoxData().getFont(),
                            PLAYER_COLUMN_WIDTH + TEXT_OFFSET);
        ...
        magics_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "magics_used"),
                        9,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++)
            magics_labels[i + 1] =
        ...
        total_labels[0] =
                new SortedLabel(
                        Utils.getBundleString(bundle, "total"),
                        10,
                        Skin.getSkin().getMultiColumnComboBoxData().getFont());
        for (int i = 0; i < players.length; i++) {
        ...
        group_buttons.setPos(
                (getWidth() - group_buttons.getWidth()) / 2,
