...
5. CI computes the version (e.g., `v2.0.3-102` for the 3rd commit after `v2.0.0-102`) → builds → tags → creates a prerelease GitHub Release with all platform artifacts attached → pushes to Steam's `prerelease` branch.
6. Promote on Steam (move the prerelease build to default) when you're ready for players to see it.
