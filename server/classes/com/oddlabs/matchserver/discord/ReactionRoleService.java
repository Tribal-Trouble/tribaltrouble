...
public class ReactionRoleService {
    ...
    private Mono<Void> handleReactionAdd(ReactionAddEvent event) {
        return processReaction(event.getUserId(), event.getEmoji(), event.getGuildId().orElse(null), event.getMessageId(), true);
    }

    private Mono<Void> handleReactionRemove(ReactionRemoveEvent event) {
        return processReaction(event.getUserId(), event.getEmoji(), event.getGuildId().orElse(null), event.getMessageId(), false);
    }

    private Mono<Void> processReaction(Snowflake userId, ReactionEmoji emoji, Snowflake guildId, Snowflake eventMessageId, boolean adding) {
        ...
        return gateway.getGuildById(guildId)
            .flatMap(guild -> guild.getMemberById(userId)
                .flatMap(member -> {
                    ...
                    if (adding) {
                        return addRoleToMember(member, roleSnowflake, guild);
                    } else {
                        return removeRoleFromMember(member, roleSnowflake, guild);
                    }
                }))
            .onErrorResume(error -> {
                System.err.println("Error processing reaction role: " + error.getMessage());
                return Mono.empty();
            });
    }
        ...
        return guild.getRoleById(roleId)
            .flatMap(role -> {
                ...
                return member.addRole(roleId)
                    .doOnSuccess(v -> System.out.println("Added role " + role.getName() + " to user " + member.getDisplayName()))
                    .doOnError(error -> System.err.println("Failed to add role: " + error.getMessage()));
            })
            .onErrorResume(error -> {
                System.err.println("Role not found or error adding role: " + error.getMessage());
                return Mono.empty();
            });
    }
        ...
        return guild.getRoleById(roleId)
            .flatMap(role -> {
                ...
                return member.removeRole(roleId)
                    .doOnSuccess(v -> System.out.println("Removed role " + role.getName() + " from user " + member.getDisplayName()))
                    .doOnError(error -> System.err.println("Failed to remove role: " + error.getMessage()));
            })
            .onErrorResume(error -> {
                System.err.println("Role not found or error removing role: " + error.getMessage());
                return Mono.empty();
            });
    }
