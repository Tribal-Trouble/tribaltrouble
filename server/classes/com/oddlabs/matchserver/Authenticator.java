...
public final strictfp class Authenticator
        implements MatchmakingServerLoginInterface, ConnectionInterface {
    ...
    private final ARMIInterfaceMethods interface_methods =
            new ARMIInterfaceMethods(MatchmakingServerLoginInterface.class);
    ...
    public static final void checkUsername(String name) throws InvalidUsernameException {
        ...
        for (int i = 0; i < name.length(); i++) {
            ...
            if (allowed_chars.indexOf(c) == -1)
                throw new InvalidUsernameException(
                        MatchmakingClientInterface.USERNAME_ERROR_INVALID_CHARACTERS);
        }
    }
    ...
    public final void createUser(
        ...
        // Check version first (before authentication checks)
        ...
        if (ServerConfiguration.getInstance().isSteamOnlyAuth()) {
            ...
            MatchmakingServer.getLogger()
                    .info("Username/password authentication disabled - Steam-only mode enabled");
            return;
        }
    ...
    public final void loginAsGuest(int revision) {
        ...
        if (ServerConfiguration.getInstance().isSteamOnlyAuth()) {
            client_interface.loginError(MatchmakingClientInterface.USER_ERROR_STEAM_REQUIRED);
            MatchmakingServer.getLogger()
                    .info("Guest authentication disabled - Steam-only mode enabled");
            return;
        }
    ...
    public final void steamLogin(
            long steamAccountId, String personaName, byte[] authTicket, int revision) {
        ...
        if (!SteamAuthValidator.validateTicket(steamAccountId, authTicket)) {
            ...
            MatchmakingServer.getLogger()
                    .warning(
                            "Steam auth ticket validation failed for account ID: "
                                    + steamAccountId);
            return;
        }
    ...
    private final boolean revisionOK(int revision) {
        if (revision != Compatibility.API_VERSION) {
            ...
            System.out.println(
                    "revision = "
                            + revision
                            + " | DBInterface.getSettingsInt(revision) = "
                            + DBInterface.getSettingsInt("revision"));
            return false;
        } else return true;
    }
    ...
    private final void doLogin(String username, String reg_key_encoded, int revision) {
        if (local_remote_address != null) {
            client_interface.loginOK(
                    username, new TunnelAddress(getHostID(), remote_address, local_remote_address));
            server.loginClient(
                    remote_address,
