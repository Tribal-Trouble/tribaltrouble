...
public class LeaderboardsCommand extends DiscordCommand {
    ...
    public Mono<Void> executeCommand(ChatInputInteractionEvent event) {
        ...
        long start = event.getOption(command_option_start).flatMap(ApplicationCommandInteractionOption::getValue).map(
                ApplicationCommandInteractionOptionValue::asLong).orElse(0L);
        long count = event.getOption(command_option_count).flatMap(ApplicationCommandInteractionOption::getValue).map(
                ApplicationCommandInteractionOptionValue::asLong).orElse(25L);
        ...
        try {
            for (int i = 0; i < ranks.length; i += 25) {
                printDebug(
                        "Creating embed for leaderboard " + (i + startInt) + "-" + Math.min(i + 25, ranks.length));

                EmbedCreateSpec.Builder builder = EmbedCreateSpec.builder().color(Color.BLUE).title(
                        "Leaderboards " + (i + startInt + 1) + "-" + Math.min(
                                i + startInt + 25,
    ...
    public ApplicationCommandRequest getCommand() {
        ApplicationCommandRequest leaderboardCommand = ApplicationCommandRequest.builder().name(
                command_name).description(command_description).addOption(
                        ApplicationCommandOptionData.builder().name(command_option_start).description(
                                "Rank to start showing the leaderboards from").type(
