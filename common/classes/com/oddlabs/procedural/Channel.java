...
public final strictfp class Channel {
    ...
    public final Channel fill(float value) {
        for (int y = 0; y < height; y++) for (int x = 0; x < width; x++) pixels[y][x] = value;
        return this;
    }
            ...
            for (int x = 0; x < width; x++) {
                putPixel(
                        ...
                        Tools.interpolateLinear(
                                new_min, new_max, (pixels[y][x] - min) * inv_maxmin));
            }
        }
        return this;
    }
    ...
    public final Channel[] quadSplit() {
        assert Utils.isPowerOf2(width) && Utils.isPowerOf2(height)
                : "only power of 2 sized channels";
    ...
    public final Channel quadJoin(
            Channel channel1, Channel channel2, Channel channel3, Channel channel4) {
        assert channel1.width == channel2.width
                        && channel2.width == channel3.width
                        && channel3.width == channel4.width
                        && channel1.height == channel2.height
                        && channel2.height == channel3.height
                        && channel3.height == channel4.height
                : "channels must be same size";
                ...
                if (getPixel(x, y) < 0.5f)
                    putPixel(
                            ...
                            (float)
                                    (StrictMath.pow(
                                                    2 * getPixel(x, y),
                                                    StrictMath.log(1 - gain) / StrictMath.log(0.5d))
                                            / 2f));
                else
                    putPixel(
                            x,
    ...
    public final Channel scaleDouble() {
        ...
        for (int y = 1; y < (height << 1) - 1; y++) {
            for (int x = 1; x < (width << 1) - 1; x++) {
                channel.putPixel(
                        x,
                        y,
                        filter.getPixel(x - 1, y)
                                + filter.getPixel(x + 1, y)
                                + filter.getPixel(x, y - 1)
                                + filter.getPixel(x, y + 1));
            }
        }
        ...
        for (int i = 0; i < max; i++) {
            channel.putPixel(
                    0,
                    i,
                    filter.getPixelWrap(-1, i)
                            + filter.getPixelWrap(1, i)
                            + filter.getPixelWrap(0, i - 1)
                            + filter.getPixelWrap(0, i + 1));
            channel.putPixel(
                    i,
                    0,
                    filter.getPixelWrap(i, -1)
                            + filter.getPixelWrap(i, 1)
                            + filter.getPixelWrap(i - 1, 0)
                            + filter.getPixelWrap(i + 1, 0));
            channel.putPixel(
                    max,
                    i,
                    filter.getPixelWrap(max - 1, i)
                            + filter.getPixelWrap(max + 1, i)
                            + filter.getPixelWrap(max, i - 1)
                            + filter.getPixelWrap(max, i + 1));
            channel.putPixel(
                    i,
                    max,
                    filter.getPixelWrap(i, max - 1)
                            + filter.getPixelWrap(i, max + 1)
                            + filter.getPixelWrap(i - 1, max)
                            + filter.getPixelWrap(i + 1, max));
        }
        pixels = channel.getPixels();
        width = width << 1;
    ...
    public final Channel shear(float offset) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                channel.putPixel(
                        x, y, getPixelWrap((int) (x + offset * width * ((float) y / height)), y));
            }
        }
        pixels = channel.getPixels();
        return this;
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                putPixel(
                        x,
                        y,
                        (float) StrictMath.sin(StrictMath.PI * 2 * frequency * getPixel(x, y)));
            }
        }
        return this;
    }
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                putPixel(
                        x,
                        y,
                        filter.getPixelWrap(x - 1, y)
                                + filter.getPixelWrap(x + 1, y)
                                + filter.getPixelWrap(x, y - 1)
                                + filter.getPixelWrap(x, y + 1));
            }
        }
        return this;
    ...
    public final int[] findFirst(float value) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (getPixel(x, y) == value) return new int[] {x, y};
            }
        }
        return new int[] {-1, -1};
    }

    public final Channel bump(
            Channel bumpmap, float lx, float ly, float shadow, float light, float ambient) {
        assert bumpmap.getWidth() == width && bumpmap.getHeight() == height
                : "bumpmap does not match channel size";
        Channel channel = new Channel(width, height);
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (brightness >= 0) {
                    channel.putPixelClip(
                            x,
                            y,
                            (getPixel(x, y) + brightness * light)
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow));
                } else {
                    channel.putPixelClip(
                            x,
                            y,
                            (getPixel(x, y) + brightness * (1 - ambient))
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow));
                }
            }
        }
    ...
    public final Channel bumpSpecular(
            Channel bumpmap,
            float lx,
            float ly,
            float lz,
            ...
            int specular) {
        assert bumpmap.getWidth() == width && bumpmap.getHeight() == height
                : "bumpmap size does not match layer size";
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                float costheta =
                        (brightness + nzlz)
                                / ((float) Math.sqrt(nx * nx + ny * ny + nz2)
                                        * lnorm); // can use math here, not game state affecting
                float highlight;
                if (costheta > 0) {
                    highlight =
                            (float)
                                    Math.pow(
                                            costheta,
                                            power); // can use math here, not game state affecting
                } else {
                    highlight = 0;
                }
                putPixelClip(
                ...
                channel.putPixel(
                        ...
                        StrictMath.max(
                                StrictMath.max(
                                        ...
                                        StrictMath.abs(
                                                getPixelWrap(x, y) - getPixelWrap(x + 1, y))),
                                StrictMath.max(
                                        StrictMath.abs(getPixelWrap(x, y) - getPixelWrap(x, y - 1)),
                                        StrictMath.abs(
                                                getPixelWrap(x, y) - getPixelWrap(x, y + 1)))));
            }
        }
        pixels = channel.getPixels();
        return this;
    ...
    public final Channel fftInv(Channel magni, Channel phase) {
        assert magni.width == magni.height
                        && phase.width == phase.height
                        && magni.width == phase.width
        ...
        for (int y = y_offset; y < y_offset + sprite.getHeight(); y++) {
            for (int x = x_offset; x < x_offset + sprite.getWidth(); x++) {
                ...
                putPixelWrap(
                        x,
                        y,
                        alpha_val * sprite.getPixelWrap(x - x_offset, y - y_offset)
                                + (1 - alpha_val) * getPixelWrap(x, y));
            }
        }
        return this;
    }
        ...
        for (int y = y_offset; y < y_offset + sprite.getHeight(); y++) {
            for (int x = x_offset; x < x_offset + sprite.getWidth(); x++) {
                putPixelWrap(
                        ...
                        StrictMath.max(
                                getPixelWrap(x, y),
                                sprite.getPixelWrap(x - x_offset, y - y_offset)));
            }
        }
        return this;
    }
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                putPixel(
                        x,
                        y,
                        alpha_val * channel.getPixel(x, y) + (1 - alpha_val) * getPixel(x, y));
            }
        }
        return this;
    }
