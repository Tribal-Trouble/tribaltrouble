...
public final strictfp class Connection extends AbstractConnection
    ...
    private final ARMIInterfaceMethods interface_methods =
            new ARMIInterfaceMethods(ConnectionPeerInterface.class);
    private boolean writing = false;
    private boolean pinged = false;
    private SelectionKey key;
    private InetAddress local_address;

    public static final void configureChannel(SocketChannel channel) throws IOException {
    ...
    private Connection(NetworkSelector network, boolean ping_reply) {
        ...
        this.peer_interface =
                (ConnectionPeerInterface)
                        ARMIEvent.createProxy(event_writer, ConnectionPeerInterface.class);
        peer_interface.ping();
    }
    ...
    Connection(
            NetworkSelector network, SelectionKey key, ConnectionInterface connection_interface) {
        this(network, false);
    ...
    public final void doPing() {
        if (isConnected()) peer_interface.ping();
    }

    public final void ping() {
        if (ping_reply) peer_interface.ping();
        pinged = true;
    }
    ...
    private final void resetWriting() {
        assert writing;
        if (isKeyValid()) doResetWriting();
        writing = false;
    }

    private final void setWriting() {
        if (writing) return;
        if (isKeyValid()) doSetWriting();
        writing = true;
    }
    ...
    private final boolean writeEvent(ARMIEvent event) {
        ...
        // Avoid differently sized serializing (we don't care about the write_buffer contents at
        // playback anyway)
        int new_position = network.getDeterministic().log(write_buffer.position());
        write_buffer.position(new_position);
        setWriting();
    ...
    private int readDeterministic(SocketChannel channel) throws IOException {
        ...
        else num_bytes_read = network.getDeterministic().log(num_bytes_read);
        ...
        assert read_buffer.position() == new_position && !read_buffer.hasRemaining()
                : read_buffer.position() + " " + new_position + " " + !read_buffer.hasRemaining();
    ...
    private void readFromChannel(SocketChannel channel) throws IOException {
        ...
        do {
            ...
            if (num_bytes_read == -1) throw new IOException("Channel closed");
            ...
            read_buffer.compact();
                ...
                && network.getDeterministic()
                        .log(network.getDeterministic().isPlayback() || channel.isOpen()));
    }

    public final void handle() throws IOException {
        SocketChannel channel;
        if (!network.getDeterministic().isPlayback()) channel = (SocketChannel) key.channel();
        else channel = null;
        network.getDeterministic().checkpoint();
        if (network.getDeterministic()
                .log(network.getDeterministic().isPlayback() || !channel.isConnected())) {
            boolean success = false;
            IOException exception;
            try {
                if (!network.getDeterministic().isPlayback()) success = channel.finishConnect();
                exception = null;
            } catch (IOException e) {
                System.out.println("Exception: " + e);
                exception = e;
            ...
            else interest_ops = network.getDeterministic().log(key.interestOps());
            int new_ops = (interest_ops | SelectionKey.OP_READ) & ~SelectionKey.OP_CONNECT;
            if (!network.getDeterministic().isPlayback()) key.interestOps(new_ops);
            connected(
                    (InetAddress)
                            network.getDeterministic()
                                    .log(
                                            network.getDeterministic().isPlayback()
                                                    ? null
                                                    : channel.socket().getLocalAddress()));
        } else {
            network.getDeterministic().checkpoint();
            if (writing) writeToChannel(channel);
            readFromChannel(channel);
        }
    }

    private final boolean isKeyValid() {
        // double negation because we want to the common result to be false, the default logger
        // value
        return !network.getDeterministic()
                .log(network.getDeterministic().isPlayback() || !(key != null && key.isValid()));
    }
